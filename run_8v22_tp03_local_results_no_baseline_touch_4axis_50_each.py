...
BATCH = "8V22_TP03_LOCAL_RESULTS_NO_BASELINE_TOUCH_4AXIS_50_EACH"
LOCAL_RESULTS_ROOT = Path("C:\\Users\\user\\Documents\\GitHub") / "-" / "local_results"
POS_FRAC = 0.01
COST_PCT = 0.10
WARMUP = 140
MIN_BARS = 260
...
def _force_local_results_args(args):
    _forced = LOCAL_RESULTS_ROOT / BATCH
    for _attr in ("outdir", "out_dir", "output_dir", "result_dir", "results_dir", "save_dir"):
        if hasattr(args, _attr):
            setattr(args, _attr, str(_forced))
    return args
...
def variants_for_axis(axis):
    # 8V22 개선안 생성부.
    # 기준선 보존: base_spec, BASELINES, 기준선 4개 조건, entry/exit 엔진, parent gate는 건드리지 않는다.
    # 개선안만 각 축 50개씩 생성하고, TP 기대값 0.3% 이상 조건은 개선안에만 적용한다.
    ...
    for j in range(50):
        ...
        if axis == "long_main":
            if j < 10:
                out.append(_make(
                    f"8V22_long_main_{n:03d}_rr_hold_lift",
                    "rr_hold_lift_tp03",
                    rr=rr0 + 0.025 * (k + 1),
                    hold=hold0 + 2 * (k % 4),
                ...
                out.append(_make(
                    f"8V22_long_main_{n:03d}_quality_reclaim",
                ...
                out.append(_make(
                    f"8V22_long_main_{n:03d}_soft_brake",
                    "soft_brake_tp03",
                ...
                out.append(_make(
                    f"8V22_long_main_{n:03d}_shock_range_guard",
                ...
                out.append(_make(
                    f"8V22_long_main_{n:03d}_balanced_mix",
                    "balanced_mix_tp03",
                    rr=rr0 + 0.020 * (k % 6),
                    hold=hold0 + 2 * (k % 3),
            ...
            if j < 10:
                out.append(_make(
                    f"8V22_long_max_{n:03d}_mdd_soft_clip",
                    "mdd_soft_clip_tp03",
                ...
                out.append(_make(
                    f"8V22_long_max_{n:03d}_edge_quality",
                    "edge_quality_tp03",
                ...
                out.append(_make(
                    f"8V22_long_max_{n:03d}_exit_compress",
                    "exit_compress_tp03",
                ...
                out.append(_make(
                    f"8V22_long_max_{n:03d}_loss_cluster_guard",
                ...
                out.append(_make(
                    f"8V22_long_max_{n:03d}_official_balance",
            ...
            if j < 10:
                out.append(_make(
                    f"8V22_short_main_{n:03d}_dd_brake_light",
                    "dd_brake_light_tp03",
                ...
                out.append(_make(
                    f"8V22_short_main_{n:03d}_overheat_quality",
                ...
                out.append(_make(
                    f"8V22_short_main_{n:03d}_exit_keep_edge",
                    "exit_keep_edge_tp03",
                ...
                out.append(_make(
                    f"8V22_short_main_{n:03d}_spike_guard",
                    "spike_guard_tp03",
                ...
                out.append(_make(
                    f"8V22_short_main_{n:03d}_hybrid_stable",
                    "hybrid_stable_tp03",
            ...
            if j < 10:
                out.append(_make(
                    f"8V22_short_max_{n:03d}_mdd_gate",
                    "mdd_gate_tp03",
                ...
                out.append(_make(
                    f"8V22_short_max_{n:03d}_quality_squeeze",
                ...
                out.append(_make(
                    f"8V22_short_max_{n:03d}_exit_mdd_balance",
                ...
                out.append(_make(
                    f"8V22_short_max_{n:03d}_blowoff_guard",
                    "blowoff_guard_tp03",
                ...
                out.append(_make(
                    f"8V22_short_max_{n:03d}_raw_preserve_guard",
