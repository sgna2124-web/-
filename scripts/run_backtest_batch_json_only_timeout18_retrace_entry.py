...
BASE_CONFIG_PATH = ROOT / "experiments" / "base_config_json_only_timeout18_retrace_entry.json"
SINGLE_CHANGES_PATH = ROOT / "experiments" / "combinations_json_only_timeout18_retrace_entry.json"
...
def run_symbol(df: pd.DataFrame, config: dict) -> list[tuple[int, int, float, str]]:
    ...
    fill_window_bars = int(entry_refine_cfg.get("fill_window_bars") or 1)
    ...
    for i in range(start_idx, len(c) - 1):
        if not inpos:
            ...
            if long_sig:
                entry_fill = None
                if mode == "none":
                    entry_fill = (i + 1, float(o[i + 1]))
                elif mode == "atr_pullback":
                    limit_price = float(c[i] - float(retrace_atr_mult) * atr_v[i])
                    entry_fill = find_limit_fill(1, i + 1, i + fill_window_bars, limit_price, o, h, l)
                elif mode == "wick_retrace":
                    wick = max(min(o[i], c[i]) - l[i], 0.0)
                    limit_price = float(c[i] - float(wick_retrace_frac) * wick)
                    entry_fill = find_limit_fill(1, i + 1, i + fill_window_bars, limit_price, o, h, l)

                if entry_fill is not None:
                    entry_idx, entry = entry_fill
...
def main() -> None:
    ...
    if all_rows:
        out_path = ROOT / "results_json_only_timeout18_retrace_entry" / "single_changes_quick_summary.csv"
