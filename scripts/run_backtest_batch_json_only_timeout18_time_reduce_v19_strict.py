...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v19_strict.json'
COMB_ENV = os.environ.get('V19_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v19_strict_conditions.json')
...
def generate_symbol_trades(symbol: str, df: pd.DataFrame, cfg: dict):
    ...
    for i in range(start, len(c) - 1):
        if not inpos:
            ...
            prev = max(ei, i - 1)
            mfe = max(mfe, (h[prev] - ep) / riskv) if side == 1 else max(mfe, (ep - l[prev]) / riskv)
            dsp = sp
...
def evaluate_portfolio_time_axis(trades, initial_asset, position_fraction, top_n_per_timestamp=None, top_n_long_per_timestamp=None, top_n_short_per_timestamp=None):
    ...
    for ts in timestamps:
        ...
        if entry_idxs:
            long_idxs = [i for i in entry_idxs if trades[i].get('side') == 1]; short_idxs = [i for i in entry_idxs if trades[i].get('side') == -1]
            long_idxs = sorted(long_idxs, key=lambda i: trades[i].get('score', 0.0), reverse=True); short_idxs = sorted(short_idxs, key=lambda i: trades[i].get('score', 0.0), reverse=True)
...
if __name__ == '__main__':
    ...
    d = ROOT / 'results_json_only_timeout18_time_reduce_v19_strict'
    d.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(out).to_csv(d / 'single_changes_quick_summary_conditions.csv', index=False)
