...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v16_strict.json'
COMB_ENV = os.environ.get('V16_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v16_strict_rankcap.json')
    ...
    for i in range(start, len(c) - 1):
        if not inpos:
            ...
            if exit_p is not None:
                trades.append({'symbol': symbol, 'entry_ts': int(ts[ei]), 'exit_ts': int(ts[i]), 'return': float(trade_return(ep, float(exit_p), side, fee)), 'score': float(score)})
                last[side] = i
                inpos = False
    return trades


def evaluate_portfolio_time_axis(trades, initial_asset, position_fraction, top_n_per_timestamp=None):
    if not trades:
    ...
    for ts in timestamps:
        ...
        if entry_idxs:
            if top_n_per_timestamp is not None:
                entry_idxs = sorted(entry_idxs, key=lambda i: trades[i].get('score', 0.0), reverse=True)[:int(top_n_per_timestamp)]
...
if __name__ == '__main__':
    ...
    for exp in build_experiments(base, group):
        ...
        topn = cfg.get('trade_control', {}).get('top_n_per_timestamp')
        s = evaluate_portfolio_time_axis(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']), top_n_per_timestamp=topn)
        s.update({'strategy': f"{cfg['strategy_name']}_{name}", 'experiment_name': name, 'initial_asset': float(cfg['initial_asset']), 'position_fraction': float(cfg['position_fraction']), 'fee_per_side': float(cfg['fee_per_side']), 'effective_config': cfg, 'engine_mode': 'strict_time_axis_realized_equity_rank_capped'})
