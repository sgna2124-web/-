...
BASE = ROOT / 'experiments' / 'v80_short_base.json'
COMB = ROOT / 'experiments' / 'v80_short_combinations.json'
...
if __name__ == '__main__':
    base = v78mod.v56mod.v27mod.load_json(BASE)
    group = v78mod.v56mod.v27mod.load_json(COMB)
    out = []
    for exp in v78mod.v56mod.v27mod.build_experiments(base, group):
        ...
        s = v78mod.evaluate_governor(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']), cfg.get('trade_control', {}), cfg.get('governor', {}))
        s.update({'strategy': f"{cfg['strategy_name']}_{name}",'experiment_name': name,'initial_asset': float(cfg['initial_asset']),'position_fraction': float(cfg['position_fraction']),'fee_per_side': float(cfg['fee_per_side']),'effective_config': cfg,'engine_mode': 'short_lane_governor_refine'})
