...
BASE_CONFIG_PATH = ROOT / "experiments" / "base_config_json_only_timeout18_exitcraft.json"
SINGLE_CHANGES_PATH = ROOT / "experiments" / "combinations_json_only_timeout18_exitcraft.json"
...
def run_symbol(df: pd.DataFrame, config: dict) -> list[tuple[int, int, float, str]]:
    ...
    exit_refine_cfg = config.get("exit_refine", {})
    ...
    breakeven_trigger_r = exit_refine_cfg.get("breakeven_trigger_r")
    trail_activate_r = exit_refine_cfg.get("trail_activate_r")
    trail_prev_bar_lookback = int(exit_refine_cfg.get("trail_prev_bar_lookback") or 1)
    fail_fast_bars = exit_refine_cfg.get("fail_fast_bars")
    fail_fast_min_progress_r = exit_refine_cfg.get("fail_fast_min_progress_r")
    ...
    for i in range(start_idx, len(c) - 1):
        if not inpos:
            ...
            if i - 1 >= ei:
                prev_idx = i - 1
                if side == 1:
                    mfe_r = max(mfe_r, (h[prev_idx] - ep) / max(entry_risk, 1e-12))
                    if (breakeven_trigger_r is not None) and (not breakeven_active) and (mfe_r >= float(breakeven_trigger_r)):
                        sp = max(sp, ep)
                        breakeven_active = True
                    if (trail_activate_r is not None) and (mfe_r >= float(trail_activate_r)):
                        trail_active = True
                        trail_idx = max(ei, i - trail_prev_bar_lookback)
                        sp = max(sp, float(l[trail_idx]))
                else:
                    mfe_r = max(mfe_r, (ep - l[prev_idx]) / max(entry_risk, 1e-12))
                    if (breakeven_trigger_r is not None) and (not breakeven_active) and (mfe_r >= float(breakeven_trigger_r)):
                        sp = min(sp, ep)
                        breakeven_active = True
                    if (trail_activate_r is not None) and (mfe_r >= float(trail_activate_r)):
            ...
            if side == 1:
                if l[i] <= sp:
                    ...
                    exit_p = tp
                elif fail_fast_bars is not None and (i - ei) >= int(fail_fast_bars) and mfe_r < float(fail_fast_min_progress_r or 0.0) and c[i] < ep:
                    exit_p = c[i]
...
def main() -> None:
    ...
    if all_rows:
        out_path = ROOT / "results_json_only_timeout18_exitcraft" / "single_changes_quick_summary.csv"
