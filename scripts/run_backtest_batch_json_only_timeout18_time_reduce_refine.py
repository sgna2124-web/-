...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_time_reduce_refine.json'
COMB=ROOT/'experiments'/'combinations_json_only_timeout18_time_reduce_refine.json'
...
def run_symbol(df,cfg):
 ...
 trb=int(r.get('time_reduce_bars',16)); trf=float(r.get('time_reduce_to_risk_frac',0.5)); ff_b=r.get('fail_fast_bars'); ff_r=r.get('fail_fast_min_progress_r')
...
if __name__=='__main__':
 ...
 d=ROOT/'results_json_only_timeout18_time_reduce_refine'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary.csv',index=False)
