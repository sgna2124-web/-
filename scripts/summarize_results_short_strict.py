...
RESULTS_DIR = ROOT / "results_short_strict"
BASELINE_NAME = "baseline_short_strict"
...
def main() -> None:
    ...
    for exp_dir in sorted(RESULTS_DIR.iterdir()):
        ...
        rows.append(
            {
                "experiment_group": "short_strict_individual",
    ...
    if not rows:
        print("No experiment summaries found under results_short_strict/.")
        return

    df = pd.DataFrame(rows)
    baseline_df = df[df["experiment"] == BASELINE_NAME]
    if baseline_df.empty:
        raise ValueError("baseline_short_strict summary.json 이 없습니다.")
    ...
    lines = ["master_summary_short_strict", ""]
