...
BASE = ROOT / 'experiments' / 'v78_short_base.json'
COMB = ROOT / 'experiments' / 'v78_short_combinations.json'
...
def evaluate_governor(trades, initial_asset, position_fraction, tc, gov):
    ...
    max_active_cap = tc.get('max_active_cap'); base_max_per_symbol = tc.get('max_per_symbol')
    ...
    hot_hours={int(x) for x in gov.get('hot_hours_utc', [])}; quiet_hours={int(x) for x in gov.get('quiet_hours_utc', [])}
    ...
    for ts in timestamps:
        ...
        for idx in entry_idxs:
            ...
            sym=tr['symbol']; t=pd.Timestamp(tr['entry_ts']); hr=int(t.hour)
            prior_hot=count_prior(sym, t, int(gov.get('hot_prior_window_bars', 48)))
            prior_quiet=count_prior(sym, t, int(gov.get('quiet_bars', 24))) if gov.get('quiet_bars') is not None else 999
            lane='standard'
            if hr in hot_hours and prior_hot >= int(gov.get('hot_prior_required', 99)): lane='hot'
            elif hr in quiet_hours and prior_quiet == 0: lane='quiet'
            ...
            eff_entry_gap = None if (lane=='hot' and gov.get('hot_disable_entrygap')) else base_entry_gap
        ...
        has_hot=any(l=='hot' for _,l,_ in cand); has_quiet=any(l=='quiet' for _,l,_ in cand)
        filtered=[]
        for item in cand:
            _, lane, _ = item
            if has_hot and gov.get('drop_standard_when_hot') and lane=='standard': continue
            if has_quiet and gov.get('drop_standard_when_quiet') and lane=='standard': continue
...
if __name__ == '__main__':
    ...
    for exp in v56mod.v27mod.build_experiments(base, group):
        ...
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'short_lane_governor'})
