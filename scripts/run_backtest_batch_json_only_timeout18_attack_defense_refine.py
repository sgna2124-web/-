...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_attack_defense_refine.json'
COMB=ROOT/'experiments'/'combinations_json_only_timeout18_attack_defense_refine.json'
...
def run_symbol(df,cfg):
 ...
 pmode=str(r.get('protect_mode','none')); ptr=float(r.get('protect_trigger_r',1.5)); plock=float(r.get('protect_lock_r',0.5)); tatr=float(r.get('trail_atr_mult',1.0)); trb=int(r.get('time_reduce_bars',12)); trf=float(r.get('time_reduce_to_risk_frac',0.5))
 ...
 for i in range(start,len(c)-1):
  if not inpos:
   ...
   dsp=sp
   if pmode in ['breakeven','combo_be_prevbar'] and mfe>=ptr:
    dsp=max(dsp,ep) if side==1 else min(dsp,ep)
   if pmode in ['profit_lock','combo_lock_time'] and mfe>=ptr:
    dsp=max(dsp,ep+plock*riskv) if side==1 else min(dsp,ep-plock*riskv)
   if pmode in ['prevbar_trail','combo_be_prevbar'] and mfe>=ptr:
    dsp=max(dsp,l[prev]) if side==1 else min(dsp,h[prev])
   if pmode=='atr_trail' and mfe>=ptr:
    dsp=max(dsp,c[prev]-tatr*atr_v[prev]) if side==1 else min(dsp,c[prev]+tatr*atr_v[prev])
   if pmode in ['time_reduce','combo_lock_time'] and (i-ei)>=trb and mfe>0:
    dsp=max(dsp,ep-riskv*trf) if side==1 else min(dsp,ep+riskv*trf)
   sp=dsp
   if side==1: exit_p=sp if l[i]<=sp else tp if h[i]>=tp else c[i] if (i-ei)>=timeout else None
   else: exit_p=sp if h[i]>=sp else tp if l[i]<=tp else c[i] if (i-ei)>=timeout else None
...
if __name__=='__main__':
 ...
 d=ROOT/'results_json_only_timeout18_attack_defense_refine'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary.csv',index=False)
