...
BASE = ROOT / 'experiments' / 'v86_short_base.json'
COMB = ROOT / 'experiments' / 'v86_short_combinations.json'
...
def count_prior(signal_history, sym, ts, window_bars):
...
def compact_row(name, summary):
    final_return_pct = float(summary.get('final_return', 0.0)) * 100.0
    mdd_pct = abs(float(summary.get('mdd', 0.0)) * 100.0)
    cd_value = final_return_pct / mdd_pct if mdd_pct > 0 else float('inf')
    return {
        'experiment': name,
...
def state_flags(gov, prior_hot, dense_count, prior_quiet):
    hot_mode = str(gov.get('hot_state_mode', 'prior_or_dense'))
    hot_by_prior = prior_hot >= int(gov.get('hot_prior_required', 99))
    hot_thr = gov.get('hot_dense_ts_threshold')
    hot_by_dense = (hot_thr is not None) and (dense_count >= int(hot_thr))
    if hot_mode == 'both':
        is_hot = hot_by_prior and hot_by_dense
    elif hot_mode == 'prior_only':
        is_hot = hot_by_prior
    elif hot_mode == 'dense_only':
        is_hot = bool(hot_by_dense)
    else:
        is_hot = hot_by_prior or bool(hot_by_dense)
    quiet_dense_max = gov.get('quiet_dense_ts_max')
    quiet_by_dense = True if quiet_dense_max is None else dense_count <= int(quiet_dense_max)
    quiet_enabled = gov.get('quiet_bars') is not None
    is_quiet = quiet_enabled and (prior_quiet == 0) and quiet_by_dense
    return is_hot, is_quiet


def evaluate_state_governor(trades, initial_asset, position_fraction, tc, gov):
    if not trades:
    ...
    base_max_per_symbol = tc.get('max_per_symbol')
    ...
    base_reentry_gap = tc.get('reentry_gap_after_close_bars')
    base_entry_gap = tc.get('entry_gap_same_symbol_bars')
    ...
    for ts in timestamps:
        ...
        normal_exit = [i for i in exit_idxs if trades[i]['entry_ts'] < ts]
        same_bar_exit = [i for i in exit_idxs if trades[i]['entry_ts'] == ts]
        for idx in normal_exit:
            close_idx(idx)
        ...
        short_pool = [i for i in entry_idxs if trades[i].get('side') == -1 and trades[i].get('score', 0.0) >= score_min_short]
        dense_count = len(short_pool)
        cand = []

        for idx in short_pool:
            tr = trades[idx]
            sym = tr['symbol']
            t = pd.Timestamp(tr['entry_ts'])
            prior_hot = count_prior(signal_history, sym, t, gov.get('hot_prior_window_bars', 48))
            prior_quiet = count_prior(signal_history, sym, t, gov.get('quiet_bars'))
            is_hot, is_quiet = state_flags(gov, prior_hot, dense_count, prior_quiet)
            lane = 'hot' if is_hot else 'quiet' if is_quiet else 'standard'

            eff_max_per_symbol = None if ((lane == 'hot' and gov.get('hot_relax_symbol_cap')) or (lane == 'quiet' and gov.get('quiet_relax_symbol_cap'))) else base_max_per_symbol
            eff_reentry_gap = None if ((lane == 'hot' and gov.get('hot_disable_reentry')) or (lane == 'quiet' and gov.get('quiet_disable_reentry'))) else base_reentry_gap
            eff_entry_gap = None if ((lane == 'hot' and gov.get('hot_disable_entrygap')) or (lane == 'quiet' and gov.get('quiet_disable_entrygap'))) else base_entry_gap
            ...
            if eff_reentry_gap is not None and sym in last_exit_ts_by_symbol and bars_diff(t, last_exit_ts_by_symbol[sym]) < int(eff_reentry_gap):
                continue
            cand.append((idx, lane, eff_max_per_symbol))

        if skip_dense_ts_threshold is not None and dense_count > int(skip_dense_ts_threshold):
            cand = []
        ...
        has_hot = any(l == 'hot' for _, l, _ in cand)
        has_quiet = any(l == 'quiet' for _, l, _ in cand)
        filtered = []
        for item in cand:
            _, lane, _ = item
            if has_hot and gov.get('drop_standard_when_hot') and lane == 'standard':
                continue
            if has_quiet and gov.get('drop_standard_when_quiet') and lane == 'standard':
                continue
            filtered.append(item)

        ranked = sorted(filtered, key=lambda x: ({'hot': 0, 'quiet': 1, 'standard': 2}.get(x[1], 9), -trades[x[0]].get('score', 0.0)))
        ...
        for idx, lane, eff_max_per_symbol in ranked:
            if max_active_cap is not None and len(active) + len(final) >= int(max_active_cap):
                break
            sym = trades[idx]['symbol']
            current_sym = symbol_counts.get(sym, 0) + sum(1 for j, _, _ in final if trades[j]['symbol'] == sym)
            if eff_max_per_symbol is not None and current_sym >= int(eff_max_per_symbol):
                continue
            final.append((idx, lane, eff_max_per_symbol))

        selected_set = {idx for idx, _, _ in final}
        for idx, _, _ in final:
            tr = trades[idx]
            active[idx] = {'notional': equity * float(position_fraction), 'entry_ts': tr['entry_ts'], 'symbol': tr['symbol'], 'side': tr['side']}
            executed.add(idx)
    ...
    return {
        ...
        'peak_growth': float(peak_growth),
        'win_rate': float(wins / len(executed)) if executed else 0.0,
        'pf': float(pf),
        'mdd': float(mdd),
        'max_conc': int(max_conc),
        'max_conc_unique_symbols': int(max_conc_symbols),
        'same_bar_trades': int(same_bar_trades),
        'active_leftover': int(len(active)),
    }
...
def run_batch():
    ...
    for exp in v56mod.v27mod.build_experiments(base, group):
        ...
        s = evaluate_state_governor(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']), cfg.get('trade_control', {}), cfg.get('governor', {}))
        s.update({
            ...
            'engine_mode': 'short_state_governor',
        })
        ...
        compact.append(compact_row(name, s))

    result_dir = ROOT / base['results_dir']
    result_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(out).to_csv(result_dir / 'lane_summary.csv', index=False)
    compact_sorted = sorted(compact, key=lambda x: (-x['cd_value'], x['mdd_pct'], -x['final_return_pct']))
    (result_dir / 'master_summary.json').write_text(json.dumps(compact_sorted, ensure_ascii=False, indent=2), encoding='utf-8')
    return compact_sorted
