...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v27_strict_score_replace_topn.json'
COMB_ENV = os.environ.get('V27_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v27_strict_score_replace_topn.json')
...
def compute_scores(close_i, ema_i, rsi_i, wick_i, body_i, atr_i, dev_thr, rsi_thr, side, tc):
    dev_cap = float(tc.get('score_dev_cap', 2.0))
    rsi_cap = float(tc.get('score_rsi_cap', 2.0))
    wick_cap = float(tc.get('score_wick_cap', 2.5))
    w_dev = float(tc.get('score_dev_weight', 1.0))
    w_rsi = float(tc.get('score_rsi_weight', 0.8))
    w_wick = float(tc.get('score_wick_weight', 0.7))
    wick_atr_floor_mult = float(tc.get('wick_atr_floor_mult', 0.15))
    if side == 1:
        raw_dev = max(0.0, -((close_i / ema_i) - 1.0))
        raw_rsi = max(0.0, (rsi_thr - rsi_i))
    else:
        raw_dev = max(0.0, ((close_i / ema_i) - 1.0))
        raw_rsi = max(0.0, (rsi_i - rsi_thr))
    dev_score = clip(raw_dev / max(dev_thr, 1e-12), 0.0, dev_cap)
    rsi_score = clip(raw_rsi / 10.0, 0.0, rsi_cap)
    denom = max(abs(body_i), atr_i * wick_atr_floor_mult, 1e-12)
    wick_ratio = max(0.0, wick_i / denom)
    wick_score = clip(math.log1p(wick_ratio), 0.0, wick_cap)
    score = w_dev * dev_score + w_rsi * rsi_score + w_wick * wick_score
    return float(score)
        ...
        if not inpos:
            ...
            if ls or ss:
                ...
                if side==1:
                    ...
                    score=compute_scores(c[i], ema_v[i], rsi_v[i], f['lower_wick'][i], f['body'][i], atr_v[i], ld, lr, 1, t)
                else:
                    ...
                    score=compute_scores(c[i], ema_v[i], rsi_v[i], f['upper_wick'][i], f['body'][i], atr_v[i], sd, sr, -1, t)
                if ok:
...
def evaluate_portfolio_time_axis_revalidated(trades, initial_asset, position_fraction, top_n_per_timestamp=None, top_n_long_per_timestamp=None, top_n_short_per_timestamp=None, score_min=None, score_min_long=None, score_min_short=None):
    if not trades:
    ...
    by_entry={}; by_exit={}
    for idx,tr in enumerate(trades): by_entry.setdefault(tr['entry_ts'],[]).append(idx); by_exit.setdefault(tr['exit_ts'],[]).append(idx)
    ...
    score_min_all = safe_threshold(score_min)
    score_min_long_value = score_min_all if score_min_long is None else float(score_min_long)
    score_min_short_value = score_min_all if score_min_short is None else float(score_min_short)
    for ts in timestamps:
        ...
        if entry_idxs:
            long_idxs=[i for i in entry_idxs if trades[i].get('side')==1 and trades[i].get('score',0.0)>=score_min_long_value]
            short_idxs=[i for i in entry_idxs if trades[i].get('side')==-1 and trades[i].get('score',0.0)>=score_min_short_value]
            ...
            if top_n_long_per_timestamp is not None: long_idxs=long_idxs[:int(top_n_long_per_timestamp)]
            if top_n_short_per_timestamp is not None: short_idxs=short_idxs[:int(top_n_short_per_timestamp)]
            selected=long_idxs+short_idxs
            if top_n_per_timestamp is not None and len(selected)>int(top_n_per_timestamp): selected=sorted(selected,key=lambda i: trades[i].get('score',0.0), reverse=True)[:int(top_n_per_timestamp)]
            selected_set=set(selected)
            for idx in selected:
                tr=trades[idx]; notional=equity*float(position_fraction); active[idx]={'notional':notional,'entry_ts':tr['entry_ts'],'symbol':tr['symbol']}; executed.add(idx)
...
if __name__=='__main__':
    base=load_json(BASE); group=load_json(COMB); out=[]
    for exp in build_experiments(base,group):
        cfg=exp['effective_config']; name=exp['name']; data_dir=Path(os.environ.get('DATA_DIR', str(ROOT/cfg['data_dir']))); out_dir=ROOT/cfg['results_dir']/name; out_dir.mkdir(parents=True, exist_ok=True); all_trades=[]
        for p in sorted(data_dir.glob('*.csv')):
            df=load_df(p)
            if len(df)<int(cfg['min_bars']): continue
            all_trades.extend(generate_symbol_trades(p.stem, df, cfg))
        tc=cfg.get('trade_control',{})
        s=evaluate_portfolio_time_axis_revalidated(all_trades,float(cfg['initial_asset']),float(cfg['position_fraction']), top_n_per_timestamp=tc.get('top_n_per_timestamp'), top_n_long_per_timestamp=tc.get('top_n_long_per_timestamp'), top_n_short_per_timestamp=tc.get('top_n_short_per_timestamp'), score_min=tc.get('score_min'), score_min_long=tc.get('score_min_long'), score_min_short=tc.get('score_min_short'))
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'strict_time_axis_revalidated_hardened_absolute_score'})
    ...
    d=ROOT/base['results_dir']; d.mkdir(parents=True, exist_ok=True); pd.DataFrame(out).to_csv(d/'lane_summary.csv', index=False)
