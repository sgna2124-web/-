...
BASE_CONFIG_PATH = ROOT / "experiments" / "base_config.json"
SINGLE_CHANGES_PATH = ROOT / "experiments" / "single_changes.json"
...
def run_single_experiment(experiment: dict) -> dict:
    ...
    if top_n is not None:
        raise NotImplementedError(
            "top_n_per_timestamp는 현재 종목별 독립 실행 구조로는 정확 구현이 안 됩니다. "
            "전 종목 신호를 시각별로 모은 뒤 글로벌 선별하는 구조가 필요합니다."
        )
...
def main() -> None:
    ...
    for experiment in experiments:
        ...
        try:
            ...
            print(f"skip {experiment['name']}: {e}")

    if all_rows:
        out_path = ROOT / "results" / "single_changes_quick_summary.csv"
