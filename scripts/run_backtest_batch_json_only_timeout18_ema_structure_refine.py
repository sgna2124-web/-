...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_ema_structure_refine.json'
COMB=ROOT/'experiments'/'combinations_json_only_timeout18_ema_structure_refine.json'
...
def run_symbol(df,cfg):
    ...
    asm=float(r['atr_stop_mult']); rrm=float(r['rr_mult']); met=float(r['min_expected_tp']); timeout=int(r['timeout_bars']); smode=str(r.get('stop_mode','wick_or_atr')); tmode=str(r.get('target_mode','ema_only')); cd=int(t['cooldown_bars_same_symbol_same_side'])
    trades=[]; inpos=False; last={1:-10_000_000,-1:-10_000_000}; start=max(int(s['ema_period']),int(s['rsi_period']),int(s['atr_period']),30)
    for i in range(start,len(c)-1):
        if not inpos:
            ...
            if ls or ss:
                ...
                if ok: inpos=True; ei=i+1; ep=float(entry); sp=float(stop); tp=float(target)
        else:
...
if __name__=='__main__':
    ...
    d=ROOT/'results_json_only_timeout18_ema_structure_refine'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary.csv',index=False)
