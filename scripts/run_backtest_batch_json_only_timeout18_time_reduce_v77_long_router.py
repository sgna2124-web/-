...
BASE = ROOT / 'experiments' / 'v77_long_base.json'
COMB = ROOT / 'experiments' / 'v77_long_combinations.json'
...
def evaluate_router(trades, initial_asset, position_fraction, tc, router):
    ...
    for ts in timestamps:
        ...
        for idx in entry_idxs:
            ...
            sym=tr['symbol']; t=pd.Timestamp(tr['entry_ts']); hr=int(t.hour); prior24=count_prior(sym,t,24); prior48=count_prior(sym,t,int(router.get('prior2_window_bars',48)))
            expand=False
            if prior48 >= 2:
                if router.get('prefer_asia_expand'):
                    expand = hr in asia_hours
                elif router.get('allow_allhours_expand'):
                    expand = True
                else:
                    expand = hr in asia_hours or hr in us_hours
            eff_max_per_symbol = router.get('expand_symbol_limit') if expand else base_max_per_symbol
            eff_reentry_gap = None if (expand and router.get('expand_disable_reentry')) else reentry_gap_after_close_bars
            eff_entry_gap = None if (expand and router.get('expand_disable_entrygap')) else entry_gap_same_symbol_bars
            ...
            if eff_reentry_gap is not None and sym in last_exit_ts_by_symbol and bars_diff(t, last_exit_ts_by_symbol[sym]) < int(eff_reentry_gap):
                continue
            candidates.append((idx, expand, eff_max_per_symbol))
        ...
        ranked=sorted(candidates, key=lambda x: (0 if x[1] else 1, -trades[x[0]].get('score',0.0)))
        final=[]; symbol_counts={}
        for p in active.values(): symbol_counts[p['symbol']] = symbol_counts.get(p['symbol'],0) + 1
        for idx, expand, eff_max_per_symbol in ranked:
            ...
            final.append((idx, expand, eff_max_per_symbol))
...
if __name__ == '__main__':
    ...
    for exp in v57mod.v56mod.v27mod.build_experiments(base, group):
        ...
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'adaptive_long_router'})
