...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_time_reduce_v5.json'
PRIMARY_COMB=ROOT/'experiments'/'combinations_json_only_timeout18_time_reduce_v5.json'
INNOV_COMB=ROOT/'experiments'/'combinations_json_only_timeout18_time_reduce_v5_innovations.json'
BATCH=os.environ.get('V5_BATCH','A').upper().strip()
...
def keep_name(name:str)->bool:
 if BATCH=='A':
  return name=='atr19_t27_b17_r020_baseline' or name.startswith('atr187_') or name.startswith('atr188_') or name.startswith('atr189_')
 if BATCH=='B':
  return name.startswith('atr190_') or name.startswith('atr191_') or name.startswith('atr192_') or name.startswith('atr193_')
 if BATCH=='C':
  return name.endswith('_stable') or name.startswith('ll_') or name.startswith('ff_')
 return False
def load_experiments(base):
 primary=lj(PRIMARY_COMB)
 selected=[e for e in primary.get('experiments',[]) if keep_name(str(e.get('name','')))]
 if BATCH=='C' and INNOV_COMB.exists():
  innov=lj(INNOV_COMB)
  selected.extend([e for e in innov.get('experiments',[]) if e.get('enabled',False)])
 return expbuild(base,{'experiments':selected})
...
def run_symbol(df,cfg):
 ...
 trb=int(r.get('time_reduce_bars',17)); trf=float(r.get('time_reduce_to_risk_frac',0.25)); ff_b=r.get('fail_fast_bars'); ff_r=r.get('fail_fast_min_progress_r')
...
if __name__=='__main__':
 base=lj(BASE); experiments=load_experiments(base); out=[]
 ...
 d=ROOT/'results_json_only_timeout18_time_reduce_v5'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/f'single_changes_quick_summary_batch_{BATCH.lower()}.csv',index=False)
