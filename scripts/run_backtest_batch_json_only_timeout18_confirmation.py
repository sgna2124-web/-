...
BASE_CONFIG_PATH = ROOT / "experiments" / "base_config_json_only_timeout18_confirmation.json"
SINGLE_CHANGES_PATH = ROOT / "experiments" / "combinations_json_only_timeout18_confirmation.json"
...
def load_symbol_df(path: Path) -> pd.DataFrame:
    ...
    if first_line.startswith("version https://git-lfs.github.com/spec/"):
        raise ValueError(
            ...
            "repo-assets workflow에서는 DATA_DIR 경로의 실제 csv를 읽어야 합니다."
        )
...
def confirm_long(mode: str, i: int, o: np.ndarray, h: np.ndarray, l: np.ndarray, c: np.ndarray, v: np.ndarray, atr_v: np.ndarray, volume_mean_window: int, volume_mult: float | None, max_entry_atr_from_confirm_close: float | None) -> bool:
    ci = i + 1
    ei = i + 2
    signal_mid = (o[i] + c[i]) / 2.0
    confirm_close = c[ci]
    confirm_open = o[ci]
    confirm_ok = False

    if mode == "reclaim_confirm":
        confirm_ok = (confirm_close > signal_mid) and (confirm_close > confirm_open)
    elif mode == "followthrough_confirm":
        confirm_ok = (h[ci] > h[i]) and (confirm_close > confirm_open)
    elif mode == "volume_reclaim_confirm":
        vol_ok = True
        if volume_mult is not None and volume_mean_window > 0 and ci >= volume_mean_window:
            vol_mean = float(np.mean(v[ci - volume_mean_window + 1: ci + 1]))
            vol_ok = v[ci] >= vol_mean * float(volume_mult)
        confirm_ok = (confirm_close > signal_mid) and (confirm_close > confirm_open) and vol_ok
    elif mode == "reclaim_no_chase_confirm":
        confirm_ok = (confirm_close > signal_mid) and (confirm_close > confirm_open)
    elif mode == "followthrough_no_chase_confirm":
        confirm_ok = (h[ci] > h[i]) and (confirm_close > confirm_open)
    else:
        return True

    if confirm_ok and ("no_chase" in mode) and (max_entry_atr_from_confirm_close is not None):
        gap_atr = abs(o[ei] - confirm_close) / max(atr_v[ci], 1e-12)
        confirm_ok = gap_atr <= float(max_entry_atr_from_confirm_close)
    return confirm_ok


def confirm_short(mode: str, i: int, o: np.ndarray, h: np.ndarray, l: np.ndarray, c: np.ndarray, v: np.ndarray, atr_v: np.ndarray, volume_mean_window: int, volume_mult: float | None, max_entry_atr_from_confirm_close: float | None) -> bool:
    ci = i + 1
    ei = i + 2
    signal_mid = (o[i] + c[i]) / 2.0
    confirm_close = c[ci]
    ...
    if mode == "reclaim_confirm":
        ...
        confirm_ok = (l[ci] < l[i]) and (confirm_close < confirm_open)
    elif mode == "volume_reclaim_confirm":
        vol_ok = True
        if volume_mult is not None and volume_mean_window > 0 and ci >= volume_mean_window:
            vol_mean = float(np.mean(v[ci - volume_mean_window + 1: ci + 1]))
...
def run_symbol(df: pd.DataFrame, config: dict) -> list[tuple[int, int, float, str]]:
    ...
    mode = str(conf_cfg.get("mode", "none"))
    volume_mean_window = int(conf_cfg.get("volume_mean_window") or 20)
    volume_mult = conf_cfg.get("volume_mult")
    max_entry_atr_from_confirm_close = conf_cfg.get("max_entry_atr_from_confirm_close")

    trades: list[tuple[int, int, float, str]] = []
    inpos = False
    last_exit_idx = {1: -10_000_000, -1: -10_000_000}
    entry_delay = 2 if mode != "none" else 1
    start_idx = max(ema_period, rsi_period, atr_period, 30)

    for i in range(start_idx, len(c) - entry_delay):
        if not inpos:
            ...
            if long_sig:
                if mode != "none":
                    if not confirm_long(mode, i, o, h, l, c, v, atr_v, volume_mean_window, volume_mult, max_entry_atr_from_confirm_close):
                        continue
                ...
                if mode != "none":
                    if not confirm_short(mode, i, o, h, l, c, v, atr_v, volume_mean_window, volume_mult, max_entry_atr_from_confirm_close):
                        continue
                entry_idx = i + entry_delay
                entry = o[entry_idx]
...
def main() -> None:
    ...
    if all_rows:
        out_path = ROOT / "results_json_only_timeout18_confirmation" / "single_changes_quick_summary.csv"
