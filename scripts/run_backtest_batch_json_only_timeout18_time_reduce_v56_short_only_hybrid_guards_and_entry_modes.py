...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v56_short_only_hybrid_guards_and_entry_modes.json'
COMB = ROOT / 'experiments' / 'combinations_json_only_timeout18_time_reduce_v56_short_only_hybrid_guards_and_entry_modes.json'
...
def evaluate_hybrid(trades, initial_asset, position_fraction, tc):
    if not trades:
        return {
            'trades': 0,
            'final_asset': float(initial_asset),
            'final_return': 0.0,
            'peak_asset': float(initial_asset),
            'peak_growth': 0.0,
            'win_rate': 0.0,
            'pf': 0.0,
            'mdd': 0.0,
            'max_conc': 0,
            'max_conc_unique_symbols': 0,
            'same_bar_trades': 0,
            'active_leftover': 0,
        }
    ...
    allowed_hours = None
    if allowed_entry_hours_utc is not None:
        allowed_hours = {int(x) for x in allowed_entry_hours_utc}
    ...
    for ts in timestamps:
        ...
        for idx in entry_idxs:
            ...
            if allowed_hours is not None and int(t.hour) not in allowed_hours:
                continue
...
def run_batch(base_path=BASE, comb_path=COMB):
    ...
    for exp in v27mod.build_experiments(base, group):
        ...
        s = evaluate_hybrid(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']), tc)
        s.update({
            ...
            'engine_mode': 'strict_time_axis_hybrid_behavioral_and_entry_modes',
        })
