...
def exps(base,group): return [{'name':e['name'],'effective_config':dm(base,e.get('overrides',{}))} for e in group.get('experiments',[]) if e.get('enabled',False)]
...
def run_symbol(df,cfg):
    ...
    ld=float(q.get('long_dev') if q.get('long_dev') is not None else s['long_dev']); lr=float(q.get('long_rsi_max') if q.get('long_rsi_max') is not None else s['long_rsi_max']); lw=float(q.get('long_wick_mult') if q.get('long_wick_mult') is not None else s['long_wick_mult']); sd=float(q.get('short_dev') if q.get('short_dev') is not None else s['short_dev']); sr=float(q.get('short_rsi_min') if q.get('short_rsi_min') is not None else s['short_rsi_min']); sw=float(q.get('short_wick_mult') if q.get('short_wick_mult') is not None else s['short_wick_mult'])
    ...
    for i in range(start,len(c)-1):
        if not inpos:
            ...
            prev=max(ei,i-1)
            if side==1:
                mfe=max(mfe,(h[prev]-ep)/riskv); exit_p=sp if l[i]<=sp else tp if h[i]>=tp else c[i] if (ff_b is not None and (i-ei)>=int(ff_b) and mfe<float(ff_r or 0.0) and c[i]<ep) or (i-ei)>=timeout else None
            else:
                mfe=max(mfe,(ep-l[prev])/riskv); exit_p=sp if h[i]>=sp else tp if l[i]<=tp else c[i] if (ff_b is not None and (i-ei)>=int(ff_b) and mfe<float(ff_r or 0.0) and c[i]>ep) or (i-ei)>=timeout else None
