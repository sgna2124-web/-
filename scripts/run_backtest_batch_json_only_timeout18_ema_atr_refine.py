...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_ema_atr_refine.json'
COMB=ROOT/'experiments'/'combinations_json_only_timeout18_ema_atr_refine.json'
...
def run_symbol(df,cfg):
 ...
 asm=float(r['atr_stop_mult']); rrm=float(r['rr_mult']); met=float(r['min_expected_tp']); timeout=int(r['timeout_bars']); smode=str(r.get('stop_mode','atr_only')); tmode=str(r.get('target_mode','ema_only')); ff_b=r.get('fail_fast_bars'); ff_r=r.get('fail_fast_min_progress_r'); cd=int(t['cooldown_bars_same_symbol_same_side'])
...
if __name__=='__main__':
 ...
 d=ROOT/'results_json_only_timeout18_ema_atr_refine'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary.csv',index=False)
