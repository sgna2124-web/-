...
BASE_CONFIG_PATH = ROOT / "experiments" / "base_config_json_only_timeout18_multimethod.json"
SINGLE_CHANGES_PATH = ROOT / "experiments" / "combinations_json_only_timeout18_multimethod.json"
...
def load_symbol_df(path: Path) -> pd.DataFrame:
    ...
    if first_line.startswith("version https://git-lfs.github.com/spec/"):
        raise ValueError(
            ...
            "repo-assets workflow에서는 DATA_DIR 환경변수 경로의 실제 csv를 읽어야 합니다."
        )
...
def run_symbol(df: pd.DataFrame, config: dict) -> list[tuple[int, int, float, str]]:
    ...
    ema_slope_lookback = quality_cfg.get("ema_slope_lookback")
    long_ema_slope_min = quality_cfg.get("long_ema_slope_min")
    short_ema_slope_max = quality_cfg.get("short_ema_slope_max")
    long_close_pos_min = quality_cfg.get("long_close_pos_min")
    short_close_pos_max = quality_cfg.get("short_close_pos_max")
    min_dev_atr = quality_cfg.get("min_dev_atr")
    max_dev_atr = quality_cfg.get("max_dev_atr")
    max_stop_pct = quality_cfg.get("max_stop_pct")

    trades: list[tuple[int, int, float, str]] = []
    inpos = False
    last_exit_idx = {1: -10_000_000, -1: -10_000_000}
    start_idx = max(ema_period, rsi_period, atr_period, 30, int(ema_slope_lookback or 0) + 1)
    ...
    for i in range(start_idx, len(c) - 1):
        if not inpos:
            ...
            ema_slope_ok_long = True
            ema_slope_ok_short = True
            if ema_slope_lookback is not None:
                prev_idx = i - int(ema_slope_lookback)
                prev_ema = ema_v[prev_idx]
                slope = 0.0 if abs(prev_ema) < 1e-12 else (ema_v[i] - prev_ema) / prev_ema
                if long_ema_slope_min is not None:
                    ema_slope_ok_long = slope >= float(long_ema_slope_min)
                if short_ema_slope_max is not None:
                    ema_slope_ok_short = slope <= float(short_ema_slope_max)

            dev_atr = abs(c[i] - ema_v[i]) / max(atr_v[i], 1e-12)
            dev_band_ok = True
            if min_dev_atr is not None:
                dev_band_ok = dev_band_ok and (dev_atr >= float(min_dev_atr))
            if max_dev_atr is not None:
                dev_band_ok = dev_band_ok and (dev_atr <= float(max_dev_atr))

            close_pos_long_ok = True if long_close_pos_min is None else feat["close_pos"][i] >= float(long_close_pos_min)
            close_pos_short_ok = True if short_close_pos_max is None else feat["close_pos"][i] <= float(short_close_pos_max)

            long_sig = (
                allow_long and long_cooldown_ok and ema_slope_ok_long and dev_band_ok and close_pos_long_ok
                and ((c[i] / ema_v[i]) - 1.0) <= -long_dev
                and rsi_v[i] < long_rsi_max
                and feat["lower_wick"][i] >= long_wick_mult * feat["body"][i]
            )
            short_sig = (
                allow_short and short_cooldown_ok and ema_slope_ok_short and dev_band_ok and close_pos_short_ok
            ...
            if long_sig:
                ...
                stop_width_ok = True if max_stop_pct is None else stop_pct <= float(max_stop_pct)
                if entry > stop and stop_width_ok and (target - entry) / entry >= min_expected_tp:
                    inpos = True
                    side = 1
                    ei = i + 1
                    ep = float(entry)
                ...
                stop_pct = (stop - entry) / max(entry, 1e-12)
                stop_width_ok = True if max_stop_pct is None else stop_pct <= float(max_stop_pct)
                if stop > entry and stop_width_ok and (entry - target) / entry >= min_expected_tp:
                    inpos = True
                    side = -1
                    ei = i + 1
                    ep = float(entry)
...
def main() -> None:
    ...
    if all_rows:
        out_path = ROOT / "results_json_only_timeout18_multimethod" / "single_changes_quick_summary.csv"
