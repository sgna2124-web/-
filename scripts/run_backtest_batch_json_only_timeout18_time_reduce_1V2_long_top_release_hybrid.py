...
BASE = ROOT / 'experiments' / '1V2_long_top_release_hybrid_base.json'
COMB = ROOT / 'experiments' / '1V2_long_top_release_hybrid_combinations.json'
...
def evaluate_state_release(trades, initial_asset, position_fraction, tc, gov):
    if not trades:
    ...
    score_min_long = v56mod.safe_threshold(tc.get('score_min')) if tc.get('score_min_long') is None else float(tc.get('score_min_long'))
    ...
    for ts in timestamps:
        ...
        long_pool = [i for i in entry_idxs if trades[i].get('side') == 1 and trades[i].get('score', 0.0) >= score_min_long]
        dense_count = len(long_pool)
        cand = []
        ...
        for idx in long_pool:
            ...
            prior_release = count_prior(signal_history, sym, t, gov.get('release_prior_window_bars', 48))
            recent_quiet = count_prior(signal_history, sym, t, gov.get('release_quiet_bars', 12))
            dense_ok_thr = gov.get('release_dense_ts_max')
            dense_ok = True if dense_ok_thr is None else dense_count <= int(dense_ok_thr)
            is_release = prior_release >= int(gov.get('release_prior_required', 99)) and recent_quiet == 0 and dense_ok
            lane = 'release' if is_release else 'standard'

            eff_max_per_symbol = None if (lane == 'release' and gov.get('release_relax_symbol_cap')) else base_max_per_symbol
            eff_reentry_gap = None if (lane == 'release' and gov.get('release_disable_reentry')) else base_reentry_gap
            eff_entry_gap = None if (lane == 'release' and gov.get('release_disable_entrygap')) else base_entry_gap
        ...
        has_release = any(l == 'release' for _, l, _ in cand)
        filtered = []
        for item in cand:
            _, lane, _ = item
            if has_release and gov.get('drop_standard_when_release') and lane == 'standard':
                continue
            filtered.append(item)

        ranked = sorted(filtered, key=lambda x: ({'release': 0, 'standard': 1}.get(x[1], 9), -trades[x[0]].get('score', 0.0)))
...
def run_batch():
    ...
    for exp in v56mod.v27mod.build_experiments(base, group):
        ...
        s = evaluate_state_release(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']), cfg.get('trade_control', {}), cfg.get('governor', {}))
        s.update({
            ...
            'engine_mode': 'long_state_release'
        })
