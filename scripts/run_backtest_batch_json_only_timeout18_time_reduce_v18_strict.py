...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v18_strict.json'
COMB_ENV = os.environ.get('V18_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v18_strict_exitarch.json')
    ...
    for i in range(start, len(c) - 1):
        if not inpos:
            ...
            if exit_p is not None:
                trades.append({'symbol': symbol, 'entry_ts': int(ts[ei]), 'exit_ts': int(ts[i]), 'return': float(trade_return(ep, float(exit_p), side, fee))})
                last[side] = i; inpos = False
    return trades


def evaluate_portfolio_time_axis(trades, initial_asset, position_fraction):
    if not trades:
    ...
    for idx, tr in enumerate(trades):
        entries.append((tr['entry_ts'], 1, idx)); exits.append((tr['exit_ts'], 0, idx))
    events = sorted(entries + exits, key=lambda x: (x[0], x[1]))
    equity = float(initial_asset); peak = equity; peak_asset = equity; mdd = 0.0; active = {}; max_conc = 0; gp = 0.0; gl = 0.0; wins = 0
    for ts, typ, idx in events:
        tr = trades[idx]
        if typ == 0:
            pos = active.pop(idx, None)
            if pos is not None:
                pnl = pos['notional'] * tr['return']; equity += pnl; peak = max(peak, equity); peak_asset = max(peak_asset, equity); mdd = min(mdd, equity / peak - 1.0)
                if pnl > 0: wins += 1; gp += pnl
                else: gl += -pnl
        else:
            notional = equity * float(position_fraction); active[idx] = {'notional': notional, 'entry_ts': tr['entry_ts']}; max_conc = max(max_conc, len(active))
    ...
    return {'trades': len(trades), 'final_asset': float(final_asset), 'final_return': float(final_return), 'peak_asset': float(peak_asset), 'peak_growth': float(peak_growth), 'win_rate': float(wins / len(trades)) if trades else 0.0, 'pf': float(pf), 'mdd': float(mdd), 'max_conc': int(max_conc)}
...
if __name__ == '__main__':
    ...
    for exp in build_experiments(base, group):
        ...
        s = evaluate_portfolio_time_axis(all_trades, float(cfg['initial_asset']), float(cfg['position_fraction']))
        s.update({'strategy': f"{cfg['strategy_name']}_{name}", 'experiment_name': name, 'initial_asset': float(cfg['initial_asset']), 'position_fraction': float(cfg['position_fraction']), 'fee_per_side': float(cfg['fee_per_side']), 'effective_config': cfg, 'engine_mode': 'strict_time_axis_realized_equity'})
    ...
    d = ROOT / 'results_json_only_timeout18_time_reduce_v18_strict'
    d.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(out).to_csv(d / 'single_changes_quick_summary_structural.csv', index=False)
