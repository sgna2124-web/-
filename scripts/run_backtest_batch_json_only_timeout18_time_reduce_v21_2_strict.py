...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v21_2_strict.json'
COMB_ENV = os.environ.get('V21_2_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v21_2_strict_mdd.json')
...
def evaluate_portfolio_time_axis(trades, initial_asset, position_fraction, top_n_per_timestamp=None, top_n_long_per_timestamp=None, top_n_short_per_timestamp=None):
    if not trades: return {'trades':0,'final_asset':float(initial_asset),'final_return':0.0,'peak_asset':float(initial_asset),'peak_growth':0.0,'win_rate':0.0,'pf':0.0,'mdd':0.0,'max_conc':0}
    by_entry={}; by_exit={}
    ...
    equity=float(initial_asset); peak=equity; peak_asset=equity; mdd=0.0; active={}; max_conc=0; gp=0.0; gl=0.0; wins=0; executed=set()
    for ts in timestamps:
        ...
        if entry_idxs:
            ...
            for idx in selected:
                tr=trades[idx]; notional=equity*float(position_fraction); active[idx]={'notional':notional,'entry_ts':tr['entry_ts']}; executed.add(idx); max_conc=max(max_conc,len(active))
...
if __name__=='__main__':
    ...
    for exp in build_experiments(base,group):
        ...
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'strict_time_axis_realized_equity_conditioned_exit'})
    ...
    d=ROOT/'results_json_only_timeout18_time_reduce_v21_2_strict'; d.mkdir(parents=True, exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary_mdd.csv', index=False)
