...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_stopshape.json'
COMB=ROOT/'experiments'/'combinations_json_only_timeout18_stopshape.json'
...
def run_symbol(df,cfg):
    s,r,t,x=cfg['signal'],cfg['risk'],cfg['trade_control'],cfg['execution']; fee=float(cfg['fee_per_side'])
    ...
    ld,lr,lw=float(s['long_dev']),float(s['long_rsi_max']),float(s['long_wick_mult']); sd,sr,sw=float(s['short_dev']),float(s['short_rsi_min']),float(s['short_wick_mult'])
    asm=float(r['atr_stop_mult']); rrm=float(r['rr_mult']); met=float(r['min_expected_tp']); timeout=int(r['timeout_bars']); smode=str(r.get('stop_mode','wick_or_atr')); tmode=str(r.get('target_mode','ema_or_rr')); ff_b=r.get('fail_fast_bars'); ff_r=r.get('fail_fast_min_progress_r'); cd=int(t['cooldown_bars_same_symbol_same_side'])
    ...
    for i in range(start,len(c)-1):
        if not inpos:
            ...
            if ls or ss:
                ...
                if side==1:
                    wick_stop=l[i]; atr_stop=entry-atr_v[i]*asm
                    stop=min(wick_stop,atr_stop) if smode=='wick_or_atr' else wick_stop if smode=='wick_only' else atr_stop
                    ema_t=ema_v[i]; rr_t=entry+rrm*(entry-stop); target=min(ema_t,rr_t) if tmode=='ema_or_rr' and ema_t>entry else rr_t if tmode in ['ema_or_rr','rr_only'] else ema_t
                    ok=entry>stop and (target-entry)/entry>=met
                else:
                    wick_stop=h[i]; atr_stop=entry+atr_v[i]*asm
                    stop=max(wick_stop,atr_stop) if smode=='wick_or_atr' else wick_stop if smode=='wick_only' else atr_stop
                    ema_t=ema_v[i]; rr_t=entry-rrm*(stop-entry); target=max(ema_t,rr_t) if tmode=='ema_or_rr' and ema_t<entry else rr_t if tmode in ['ema_or_rr','rr_only'] else ema_t
...
if __name__=='__main__':
    ...
    d=ROOT/'results_json_only_timeout18_stopshape'; d.mkdir(parents=True,exist_ok=True); pd.DataFrame(out).to_csv(d/'single_changes_quick_summary.csv',index=False)
