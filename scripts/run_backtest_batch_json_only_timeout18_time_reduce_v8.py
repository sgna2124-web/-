...
BASE=ROOT/'experiments'/'base_config_json_only_timeout18_time_reduce_v8.json'
COMB_ENV=os.environ.get('V8_COMB','experiments/combinations_json_only_timeout18_time_reduce_v8_micro.json')
...
def run_symbol(df,cfg):
 ...
 trb=int(r.get('time_reduce_bars',17)); trf=float(r.get('time_reduce_to_risk_frac',0.16)); ff_b=r.get('fail_fast_bars'); ff_r=r.get('fail_fast_min_progress_r')
...
if __name__=='__main__':
 ...
 d=ROOT/'results_json_only_timeout18_time_reduce_v8'; d.mkdir(parents=True,exist_ok=True); tag=Path(COMB).stem.replace('combinations_json_only_timeout18_time_reduce_v8_',''); pd.DataFrame(out).to_csv(d/f'single_changes_quick_summary_{tag}.csv',index=False)
