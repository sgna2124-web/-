...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v14_strict.json'
COMB_ENV = os.environ.get('V14_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v14_strict_recheck.json')
        ...
        if not inpos:
            ...
            if exit_p is not None:
                trades.append({
                    ...
                    'entry_ts': int(ts[ei]),
                    'exit_ts': int(ts[i]),
                    'return': float(trade_return(ep, float(exit_p), side, fee)),
                })
                last[side] = i
                inpos = False
    return trades
...
if __name__ == '__main__':
    ...
    for exp in build_experiments(base, group):
        ...
        s.update({
            ...
            'engine_mode': 'strict_time_axis_realized_equity',
        })
    ...
    d = ROOT / 'results_json_only_timeout18_time_reduce_v14_strict'
    d.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(out).to_csv(d / 'single_changes_quick_summary_strict.csv', index=False)
