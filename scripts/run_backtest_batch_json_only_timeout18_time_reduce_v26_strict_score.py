...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v26_strict_score.json'
COMB_ENV = os.environ.get('V26_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v26_strict_score.json')
...
def compute_scores(close_i, ema_i, rsi_i, wick_i, body_i, atr_i, dev_thr, rsi_thr, side, tc):
    ...
    return float(score), float(dev_score), float(rsi_score), float(wick_score)
        ...
        if not inpos:
            ...
            if ls or ss:
                ...
                if side==1:
                    ...
                    score, dev_score, rsi_score, wick_score = compute_scores(c[i], ema_v[i], rsi_v[i], f['lower_wick'][i], f['body'][i], atr_v[i], ld, lr, 1, t)
                else:
                    ...
                    score, dev_score, rsi_score, wick_score = compute_scores(c[i], ema_v[i], rsi_v[i], f['upper_wick'][i], f['body'][i], atr_v[i], sd, sr, -1, t)
                if ok:
            ...
            if exit_p is not None:
                trades.append({'symbol':symbol,'entry_ts':int(ts[ei]),'exit_ts':int(ts[i]),'return':float(trade_return(ep,float(exit_p),side,fee)),'score':float(score),'side':int(side),'same_bar': bool(int(ts[ei]) == int(ts[i])),'dev_score':float(dev_score),'rsi_score':float(rsi_score),'wick_score':float(wick_score)})
