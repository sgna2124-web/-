...
BASE = ROOT / 'experiments' / 'base_config_json_only_timeout18_time_reduce_v23_strict_revalidated.json'
COMB_ENV = os.environ.get('V23_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v23_strict_revalidated.json')
...
def load_df(path: Path):
    ...
    for target, keys in alias.items():
        for k in keys:
            if k.lower() in raw.columns: mp[k.lower()] = target; break
...
def evaluate_portfolio_time_axis_revalidated(trades, initial_asset, position_fraction, top_n_per_timestamp=None, top_n_long_per_timestamp=None, top_n_short_per_timestamp=None):
    if not trades:
    ...
    for ts in timestamps:
        ...
        if entry_idxs:
            long_idxs=sorted([i for i in entry_idxs if trades[i].get('side')==1], key=lambda i: trades[i].get('score',0.0), reverse=True)
            short_idxs=sorted([i for i in entry_idxs if trades[i].get('side')==-1], key=lambda i: trades[i].get('score',0.0), reverse=True)
...
if __name__=='__main__':
    ...
    for exp in build_experiments(base,group):
        ...
        s=evaluate_portfolio_time_axis_revalidated(all_trades,float(cfg['initial_asset']),float(cfg['position_fraction']), top_n_per_timestamp=tc.get('top_n_per_timestamp'), top_n_long_per_timestamp=tc.get('top_n_long_per_timestamp'), top_n_short_per_timestamp=tc.get('top_n_short_per_timestamp'))
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'strict_time_axis_revalidated_same_ts_fixed'})
    ...
    d=ROOT/'results_json_only_timeout18_time_reduce_v23_strict_revalidated'; d.mkdir(parents=True, exist_ok=True); pd.DataFrame(out).to_csv(d/'revalidated_summary.csv', index=False)
