...
BASE_ENV = os.environ.get('V24_BASE', 'experiments/base_config_json_only_timeout18_time_reduce_v24_1_strict_revalidated.json')
COMB_ENV = os.environ.get('V24_COMB', 'experiments/combinations_json_only_timeout18_time_reduce_v24_1_strict_score.json')
BASE = (ROOT / BASE_ENV) if not Path(BASE_ENV).is_absolute() else Path(BASE_ENV)
...
def capped_ratio(wick: float, body: float, clip_value):
    ratio = float(wick / (abs(body) + 1e-12))
...
def generate_symbol_trades(symbol: str, df: pd.DataFrame, cfg: dict):
    ...
    score_clip = t.get('score_clip_wick_body')
        ...
        if not inpos:
            ...
            if ls or ss:
                ...
                if side==1:
                    ...
                    wick_score = capped_ratio(f['lower_wick'][i], f['body'][i], score_clip)
                    score=float(long_dev_now + max(0.0,(lr-rsi_v[i])/100.0) + wick_score)
                else:
                    ...
                    wick_score = capped_ratio(f['upper_wick'][i], f['body'][i], score_clip)
                    score=float(short_dev_now + max(0.0,(rsi_v[i]-sr)/100.0) + wick_score)
                if ok:
...
def evaluate_portfolio_time_axis_revalidated(trades, initial_asset, position_fraction, top_n_per_timestamp=None, top_n_long_per_timestamp=None, top_n_short_per_timestamp=None, score_min=None, score_min_long=None, score_min_short=None):
    ...
    score_min_all = None if score_min is None else float(score_min)
...
if __name__=='__main__':
    ...
    for exp in build_experiments(base,group):
        ...
        s.update({'strategy':f"{cfg['strategy_name']}_{name}",'experiment_name':name,'initial_asset':float(cfg['initial_asset']),'position_fraction':float(cfg['position_fraction']),'fee_per_side':float(cfg['fee_per_side']),'effective_config':cfg,'engine_mode':'strict_time_axis_revalidated_same_ts_fixed_score_filtered'})
