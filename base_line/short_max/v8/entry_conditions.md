...
- 변경 핵심: 진입 조건 자체가 아니라, 백테스트 체결/청산 엔진을 실제 5분봉 운용 순서에 맞춰 수정
...
각 심볼별로 한 번에 하나의 숏 포지션만 유지한다.
신호는 캔들 close 기준으로 계산하고, 실제 진입은 다음 캔들 open에서 수행한다.
...
RSI가 77보다 커야 한다는 직접 필터는 사용하지 않는다.
단, RSI는 short_score 계산 내부의 보조 점수로 사용한다.
...
dev_score = clamp(raw_dev / 0.035, 0.0, 2.0)
...
공식 재현에서는 `expected_tp >= 0.003`을 필수 검증한다.
여기에 `target > 0` 같은 임의 필터를 추가하면 재현이 깨질 수 있다.

## 청산 조건

진입 후 각 캔들에서 다음 순서로 평가한다.
...
진입 후 8봉 이상 경과했고 MFE가 0보다 크면 stop을 다음 수준 이하로 축소한다.
