...
"""
Short max v6 baseline strategy.
...
Core interpretation:
    ...
    - dd_brake mode is edge_current at portfolio level.
    - entry is next bar open.
    - fee_per_side=0.0004, position_fraction=0.01.
"""
...
class ShortMaxV6Config:
    strategy_name: str = "SM16_C05_remove_no_rsi_dev035_exact_entrymask_ddedge"
    original_name: str = "SM16_C05_remove_no_rsi_dev035"
    axis: str = "short_max"
    side: str = "short"
    ...
    current_597_trades: int = 34005
    current_597_win_rate_pct: float = 14.406704896338773
    current_597_final_return_pct: float = 898.4999401921407
    current_597_max_return_pct: float = 899.0095709169104
    current_597_max_drawdown_pct: float = 4.559023920452521
    current_597_official_cd_value: float = 953.4644856111984
    current_597_profit_factor: float = 1.555465134190124
    current_597_max_conc: int = 277
    current_597_same_bar_trades: int = 3115
    current_597_active_leftover: int = 0
    current_597_blocked_by_guard: int = 39
    current_597_generated_trades_before_score_filter: int = 34044
    current_597_errors: int = 0


CFG = ShortMaxV6Config()


def config_dict(cfg: ShortMaxV6Config = CFG) -> Dict[str, Any]:
    return asdict(cfg)
...
def add_indicators(df: pd.DataFrame, cfg: ShortMaxV6Config = CFG) -> pd.DataFrame:
    out = df.copy()
...
def entry_mask(df: pd.DataFrame, cfg: ShortMaxV6Config = CFG) -> pd.Series:
...
def build_trade(df: pd.DataFrame, signal_index: int, symbol: str = "", cfg: ShortMaxV6Config = CFG) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "score": float(sig["short_score"]),
    }


def apply_time_reduce(entry: float, risk: float, current_stop: float, bars_held: int, mfe_r: float, cfg: ShortMaxV6Config = CFG) -> float:
    if bars_held >= cfg.time_reduce_bars and mfe_r > 0:
        return min(current_stop, entry + risk * cfg.time_reduce_to_risk_frac)
    return current_stop


def exit_short(entry: float, stop: float, target: float, risk: float, mfe_r: float, bars_held: int, high: float, low: float, close: float, cfg: ShortMaxV6Config = CFG):
...
def net_short_return(entry: float, exit_price: float, cfg: ShortMaxV6Config = CFG) -> float:
...
BASELINE_NOTES = {
    ...
    "axis": CFG.axis,
    "retest_source": "local_results/short_max/short_max_v5_v3_top1_retest_results",
    "official_result": {
    ...
    "critical_rules": [
        ...
        "RSI remains inside short_score.",
        "score_min_short is included in entry mask.",
        "dd_brake is edge_current at portfolio level.",
        "entry is next bar open.",
        "same-bar trades are closed immediately.",
