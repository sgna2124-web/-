...
- 승격 사유: short_max식 기준인 MDD 10% 미만에서 기존 v10보다 official_cd_value가 높고, MDD도 낮다.
...
v11은 v10의 진입 조건을 그대로 유지하고 청산부의 stop 폭만 변경한다.
...
해석: 진입 신호는 동일하게 두고 stop 폭을 ATR 기준으로 소폭 넓혀 불필요한 조기 손절을 줄인다. target은 rr_mult 5.5가 유지되므로 risk 확대에 따라 목표 가격도 함께 멀어진다.
...
- v10보다 official_cd_value가 상승했다.
- v10보다 max_drawdown_pct가 낮아졌다.
- 거래 수는 줄었지만 profit_factor가 개선되었다.
- MDD 5% 미만이므로 short_main식 기준도 만족한다.

## 단점과 리스크

- win_rate는 10.9499%로 낮다. 소수 target 수익에 크게 의존한다.
- max_conc 310으로 동시 포지션 수가 많다.
- stop 폭을 넓힌 전략이므로 특정 급등 구간에서는 손실 포지션의 체류와 변동성이 늘 수 있다.
