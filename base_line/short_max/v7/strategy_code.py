...
"""
short_max v7 baseline strategy
...
Core change:
    score_dev_weight: 1.0 -> 1.2

Critical rules:
    - short_max-only baseline. MDD is 5.6637%, so it is not a short_main baseline.
...
class ShortMaxV7Config:
    strategy_name: str = "short_max_v7_devw120"
    candidate_name: str = "devw120"
    ...
    current_597_trades: int = 43681
    current_597_win_rate_pct: float = 13.827522263684438
    current_597_final_return_pct: float = 1221.3299878755902
    current_597_max_return_pct: float = 1221.9746135454966
    current_597_max_drawdown_pct: float = 5.6636954922983485
    current_597_official_cd_value: float = 1247.1019969487918
    current_597_profit_factor: float = 1.5021526593629504
    current_597_max_conc: int = 295
    current_597_max_conc_unique_symbols: int = 295
    current_597_same_bar_trades: int = 3694
    current_597_active_leftover: int = 0
    current_597_blocked_by_guard: int = 152
    current_597_generated_trades_before_score_filter: int = 43833
    current_597_errors: int = 0


CFG = ShortMaxV7Config()


def config_dict(cfg: ShortMaxV7Config = CFG) -> Dict[str, Any]:
    return asdict(cfg)
...
def add_indicators(df: pd.DataFrame, cfg: ShortMaxV7Config = CFG) -> pd.DataFrame:
    out = df.copy()
...
def entry_mask(df: pd.DataFrame, cfg: ShortMaxV7Config = CFG) -> pd.Series:
...
def build_trade(df: pd.DataFrame, signal_index: int, symbol: str = "", cfg: ShortMaxV7Config = CFG) -> Optional[Dict[str, Any]]:
...
def apply_time_reduce(entry: float, risk: float, current_stop: float, bars_held: int, mfe_r: float, cfg: ShortMaxV7Config = CFG) -> float:
    if bars_held >= cfg.time_reduce_bars and mfe_r > 0:
        return min(current_stop, entry + risk * cfg.time_reduce_to_risk_frac)
    return current_stop


def exit_short(entry: float, stop: float, target: float, risk: float, mfe_r: float, bars_held: int, high: float, low: float, close: float, cfg: ShortMaxV7Config = CFG):
...
def net_short_return(entry: float, exit_price: float, cfg: ShortMaxV7Config = CFG) -> float:
...
BASELINE_NOTES = {
    ...
    "critical_rules": [
        "short_max-only baseline; not short_main because MDD exceeds 5%.",
