...
- 승격 사유: short_max식 기준인 MDD 10% 미만에서 기존 v11보다 official_cd_value가 높고, MDD도 낮다. MDD 5% 미만이므로 short_main식 기준도 통과한다.
...
v12는 v11의 진입 조건을 그대로 유지하고 청산부만 조정한다.

- atr_stop_mult: 2.10 -> 2.15
- rr_mult: 5.5 -> 5.4
- time_reduce_bars: 4 유지
- timeout_bars: 200 유지
- fail_fast_bars: 10 유지

해석: stop 폭을 ATR 기준으로 더 넓히고, RR 목표는 5.5에서 5.4로 약간 당긴다. 결과적으로 불필요한 조기 손절은 줄이고, 목표가 도달 효율을 높이는 구조다.
...
- v11보다 official_cd_value가 크게 상승했다.
- v11보다 max_drawdown_pct가 낮아졌다.
- 거래 수는 줄었지만 profit_factor가 개선되었다.
- MDD 5% 미만이므로 short_main식 기준도 만족한다.
- rr을 5.4로 당겼음에도 max_return과 CD가 모두 상승했다.

## 단점과 리스크

- win_rate는 11.0001%로 낮다. 소수 target 수익에 크게 의존한다.
- max_conc 310으로 동시 포지션 수가 많다.
- stop 폭 2.15는 v11보다 넓으므로 급등 구간 손실 포지션의 체류 변동성이 커질 수 있다.
