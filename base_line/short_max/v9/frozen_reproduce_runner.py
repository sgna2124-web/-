...
CFG=dict(strategy='smv8_mix2_13_all_timereduce5',axis='short_max',baseline_version='short_max/v9',initial_asset=100.0,position_fraction=0.01,fee_per_side=0.0004,min_bars=120,ema_period=20,rsi_period=14,atr_period=14,short_dev=0.032,short_rsi_min=77.0,use_rsi_gate=False,short_wick_mult=1.3,score_min_short=2.35,score_dev_weight=1.3,score_rsi_weight=0.8,score_wick_weight=0.7,score_dev_cap=2.0,score_rsi_cap=2.0,score_wick_cap=2.5,wick_atr_floor_mult=0.2,atr_stop_mult=2.0,rr_mult=5.5,min_expected_tp=0.003,timeout_bars=200,time_reduce_bars=5,time_reduce_to_risk_frac=0.05,fail_fast_bars=10,fail_fast_min_progress_r=0.1,dd_brake_trigger_pct=0.03,dd_brake_freeze_steps=5,dd_brake_mode='edge_current')
EXPECTED=dict(trades=63105,max_return_pct=2743.3304850694603,max_drawdown_pct=5.686879318598392,official_cd_value=2681.6337117546423,active_leftover=0,pending_leftover=0,load_errors=0)
OLD=dict(strategy='short_max_v7_devw120_actual_bar_engine',trades=45500,max_return_pct=1424.4317435070927,max_drawdown_pct=6.104584306764704,official_cd_value=1431.3715225256192)
...
def main():
    ...
    dd=data_dir(a.data_dir); out=Path(a.out_dir).expanduser().resolve() if a.out_dir else Path(__file__).resolve().parent/'short_max_v9_frozen_reproduce_results'; out.mkdir(parents=True,exist_ok=True)
