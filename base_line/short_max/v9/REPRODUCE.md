...
처음 보는 사람이 short_max v9 공식 기준선을 재현하기 위한 최소 실행 절차를 기록한다.
...
```text
base_line/short_max/v9/short_max_v9_frozen_reproduce_results
```
...
short_max/v9/frozen_reproduce_runner.py는 완전 독립형 단일 파일이다.
...
따라서 short_max/v9 폴더만 복사해도 같은 OHLCV 데이터셋이 있으면 재현 가능해야 한다.
