"""
short_max baseline strategy code

Strategy: short_only_reference_1x
Axis: short_max
Purpose: baseline strategy definition for future short_max development.
...
Use it as the source of truth when embedding the short_max baseline into a backtest script.
"""
...
class ShortMaxBaselineConfig:
    name: str = "short_only_reference_1x"
    axis: str = "short_max"
    side: str = "short"
    ...
    dd_brake_trigger_pct: float = 0.0
    dd_brake_freeze_steps: int = 0

    cooldown_bars_same_symbol_same_side: int = 0

    current_597_trades: int = 36834
    current_597_max_return_pct: float = 408.9954916988155
    current_597_max_drawdown_pct: float = 7.395550425783604
    current_597_official_cd_value: float = 471.3524734452645


def config_dict() -> Dict[str, Any]:
    return asdict(ShortMaxBaselineConfig())


def add_indicators(df: pd.DataFrame, cfg: ShortMaxBaselineConfig = ShortMaxBaselineConfig()) -> pd.DataFrame:
...
def short_max_entry_mask(df: pd.DataFrame, cfg: ShortMaxBaselineConfig = ShortMaxBaselineConfig()) -> pd.Series:
...
def build_short_trade_from_signal(
    df: pd.DataFrame,
    signal_index: int,
    cfg: ShortMaxBaselineConfig = ShortMaxBaselineConfig(),
...
def dd_brake_disabled() -> bool:
    """short_max baseline has no dd_brake."""
    return True


BASELINE_CONFIG = ShortMaxBaselineConfig()
