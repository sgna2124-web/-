...
따라서 v3은 신규 진입 조건 전략이 아니라, 기준선 진입 조건을 유지한 리스크 파라미터 개선 전략이다.
...
따라서 v3 TP03은 v2보다 약간 달라진다. 진입 entry_key 이름은 같지만, TP03 계산에 들어가는 atr_stop이 1.10으로 바뀌므로 실제 진입 수가 v2와 달라질 수 있다.
...
- v2의 `atr_stop = 1.01`을 쓰면 안 된다.
- v3은 `atr_stop = 1.10`이다.
- entry_key는 같아도 TP03 계산에 atr_stop이 들어가므로 v2와 진입 수가 달라진다.
