...
따라서 v4는 신규 진입 조건 전략이 아니라, 기준선 진입 조건을 유지한 수익 목표 배율 개선 전략이다.
...
따라서 v4 TP03은 v3보다 약간 달라진다. 진입 entry_key 이름은 같지만, TP03 계산에 들어가는 rr_target이 3.20으로 바뀌므로 실제 진입 수가 v3와 달라질 수 있다.
...
- v3의 `rr_target = 2.90`을 쓰면 안 된다.
- v4는 `rr_target = 3.20`이다.
- entry_key는 같아도 TP03 계산에 rr_target이 들어가므로 v3와 진입 수가 달라진다.
