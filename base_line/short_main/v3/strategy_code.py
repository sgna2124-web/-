"""
short_main v3 baseline strategy code

Strategy: SM15_B10_rr575_tr8_f005
Parent baseline: short_beh_dd_brake
Axis: short_main
Purpose: official short_main v3 baseline definition after v1.5 SM24-neighborhood development.

This file intentionally contains only strategy-side logic and parameters.
It does not import any external runner, json config, or repository-specific path.
Use it as the source of truth when embedding the short_main v3 baseline into a backtest script.
"""
...
class ShortMainV3BaselineConfig:
    name: str = "SM15_B10_rr575_tr8_f005"
    parent: str = "short_beh_dd_brake"
    axis: str = "short_main"
    side: str = "short"
    ...
    # Entry condition inherited from short_beh_dd_brake.
    ...
    current_597_trades: int = 28313
    current_597_max_return_pct: float = 331.3434403627648
    current_597_max_drawdown_pct: float = 3.9772559021280185
    current_597_official_cd_value: float = 414.18780792249464
    current_597_pf: float = 1.4757306514967135
    current_597_max_conc: int = 286


def config_dict() -> Dict[str, Any]:
    return asdict(ShortMainV3BaselineConfig())


def add_indicators(df: pd.DataFrame, cfg: ShortMainV3BaselineConfig = ShortMainV3BaselineConfig()) -> pd.DataFrame:
...
def short_main_v3_entry_mask(df: pd.DataFrame, cfg: ShortMainV3BaselineConfig = ShortMainV3BaselineConfig()) -> pd.Series:
...
def build_short_trade_from_signal(
    df: pd.DataFrame,
    signal_index: int,
    cfg: ShortMainV3BaselineConfig = ShortMainV3BaselineConfig(),
...
def apply_time_reduce_stop(
    ...
    After 8 bars and positive MFE, reduce stop toward entry + 0.05R.
    """
...
def should_fail_fast(
    ...
    """Short-side fail_fast condition."""
    return bool(
        bars_held >= cfg.fail_fast_bars
        and mfe_r < cfg.fail_fast_min_progress_r
        and close > entry
    )


def apply_dd_brake_state(current_drawdown_pct: float, freeze_left: int, cfg: ShortMainV3BaselineConfig = ShortMainV3BaselineConfig()) -> int:
    """
...
BASELINE_CONFIG = ShortMainV3BaselineConfig()
