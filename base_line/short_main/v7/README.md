...
이 버전은 기존 `short_main v6`의 진입 조건을 유지하되, 백테스트 체결/청산 시간 처리 엔진을 실제 5분봉 운용 방식에 맞춘 `actual bar engine`으로 교체한 버전이다.

따라서 전략 조건만 보면 v6 계열의 연장선이지만, 진입/청산 처리 방식이 바뀌었으므로 새 기준선 버전 `v7`로 기록한다.
...
- source_old_strategy: `short_main_v6_timeout210`
...
- final_asset: 1214.6701489565148
- final_return_pct: 1114.6701489565148
- peak_asset: 1215.0033786152128
...
- pending_leftover: 0
- blocked_by_guard: 39
- generated_entry_candidates: 35,369
- executed_entries: 35,330
- load_errors: 0

## 이전 구엔진 기준선 대비

- old_engine_strategy: `short_main_v6_timeout210`
- old_engine_trades: 33,989
- old_engine_max_return_pct: 931.6464095007982
- old_engine_max_drawdown_pct: 4.506694290977831
- old_engine_official_cd_value: 985.153259660748

차이:

- delta_cd_vs_old_engine: +173.8670166736598
- delta_mdd_vs_old_engine: +0.10095563544553254
- delta_trades_vs_old_engine: +1,341
...
- short_main 기준인 MDD 5% 미만을 유지하면서 CD가 구엔진 기준선보다 상승했다.
- active_leftover와 pending_leftover가 모두 0이라 기준선 기록 조건을 만족한다.

## 단점과 주의사항

- MDD가 구엔진 대비 0.10095563544553254%p 증가했다.
- win_rate는 14.143% 수준으로 낮다. 수익 구조는 낮은 승률과 큰 RR에 의존한다.
- 같은 조건이라도 엔진 순서가 달라지면 거래 수와 성과가 크게 달라진다.
- 이 기준선은 2025년까지의 train 기준이다. 2026 holdout 검증은 별도로 수행해야 한다.
- `strategy_code.py`만 복사해서는 재현이 부족하다. 반드시 동일한 actual bar engine을 사용해야 한다.

## 기준선 판정

short_main v7은 short_main 공식 기준선으로 승격한다.
