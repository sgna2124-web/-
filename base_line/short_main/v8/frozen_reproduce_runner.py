...
"""
short_main v8 frozen reproduction runner

목적
- 처음 보는 사람이 short_main v8 기준선을 그대로 재현할 수 있게 하는 단독 실행 파일이다.
- 외부 runner, 외부 json, GitHub 저장소 경로를 참조하지 않는다.
- OHLCV CSV 폴더만 지정하면 v8 공식 결과를 재현하도록 actual bar engine을 포함한다.

실행 예시
python base_line/short_main/v8/frozen_reproduce_runner.py --data-dir ./Data/time
python base_line/short_main/v8/frozen_reproduce_runner.py --data-dir ./Data/time --out-dir ./local_results/short_main/SHORT_MAIN_V8_REPRODUCE
...
def main() -> None:
    ap = argparse.ArgumentParser(description="short_main v8 frozen reproduction runner")
    ap.add_argument("--data-dir", default=None)
    ap.add_argument("--out-dir", default="./local_results/short_main/SHORT_MAIN_V8_REPRODUCE")
    ...
    print("[START] short_main v8 frozen reproduction")
    ...
    pd.DataFrame([result]).to_csv(out_dir / "short_main_v8_reproduction_summary.csv", index=False, encoding="utf-8-sig")
    (out_dir / "short_main_v8_reproduction_summary.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "short_main_v8_reproduction_errors.json").write_text(json.dumps(errors, ensure_ascii=False, indent=2), encoding="utf-8")
    metadata = dict(
        script="base_line/short_main/v8/frozen_reproduce_runner.py",
    ...
    (out_dir / "short_main_v8_reproduction_metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")

    if gate_pass:
        print("[GATE PASS] short_main v8 official baseline reproduced")
    else:
        print("[GATE FAIL] short_main v8 official baseline mismatch")
        for issue in gate_issues:
            print(" -", issue)
        if args.strict_gate:
            raise RuntimeError("short_main v8 reproduction gate failed")
    print(pd.DataFrame([result]).to_string(index=False))
    print("[DONE]", out_dir / "short_main_v8_reproduction_summary.csv")
