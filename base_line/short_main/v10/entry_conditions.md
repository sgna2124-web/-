...
RSI는 short_score 계산 내부의 보조 점수로만 사용한다.
...
- target: `entry - 5.5 * risk`
- time reduce: 진입 후 6봉 이상 경과했고 MFE가 0보다 크면 stop을 `entry + risk * 0.05` 이하로 축소
- fail fast: 진입 후 10봉 이상 경과, MFE 0.1R 미만, close가 entry보다 높으면 close 청산
