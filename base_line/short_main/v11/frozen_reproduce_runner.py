...
CFG=dict(strategy='smv9_topcombo1_01_tr4_stop205_rr550',axis='short_main',baseline_version='short_main/v11',initial_asset=100.0,position_fraction=0.01,fee_per_side=0.0004,min_bars=120,ema_period=20,rsi_period=14,atr_period=14,short_dev=0.032,short_rsi_min=77.0,use_rsi_gate=False,short_wick_mult=1.3,score_min_short=2.35,score_dev_weight=1.3,score_rsi_weight=0.8,score_wick_weight=0.7,score_dev_cap=2.0,score_rsi_cap=2.0,score_wick_cap=2.5,wick_atr_floor_mult=0.2,atr_stop_mult=2.05,rr_mult=5.5,min_expected_tp=0.003,timeout_bars=200,time_reduce_bars=4,time_reduce_to_risk_frac=0.05,fail_fast_bars=10,fail_fast_min_progress_r=0.1,atr_pct_min=0.0,atr_pct_max=999.0,body_pct_max=999.0,close_position_min=-999.0,upper_body_ratio_max=float('inf'),dd_brake_trigger_pct=0.03,dd_brake_freeze_steps=5,dd_brake_mode='edge_current')
EXPECTED=dict(trades=64339,max_return_pct=3689.4315334640614,max_drawdown_pct=4.629389056231814,official_cd_value=3614.004004760479,active_leftover=0,pending_leftover=0,load_errors=0)
OLD=dict(strategy='smv8_mix2_02_prev_mix18_top2_top3_timereduce6',trades=50501,max_return_pct=1973.4472303933733,max_drawdown_pct=4.814092666588577,official_cd_value=1973.629559329422,profit_factor=1.5675065791005796)
...
def main():
    ...
    dd=data_dir(a.data_dir); out=Path(a.out_dir).expanduser().resolve() if a.out_dir else Path(__file__).resolve().parent/'short_main_v11_frozen_reproduce_results'; out.mkdir(parents=True,exist_ok=True)
