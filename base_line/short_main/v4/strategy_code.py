"""
short_main v4 baseline strategy code

Strategy: SM16_C05_remove_no_rsi_dev035
Parent baseline: SM15_B10_rr575_tr8_f005
Grandparent baseline: short_beh_dd_brake
Axis: short_main
Purpose: official short_main v4 baseline definition after v1.6 entry-condition development.
...
Use it as the source of truth when embedding the short_main v4 baseline into a backtest script.
"""
...
class ShortMainV4BaselineConfig:
    name: str = "SM16_C05_remove_no_rsi_dev035"
    parent: str = "SM15_B10_rr575_tr8_f005"
    grandparent: str = "short_beh_dd_brake"
    axis: str = "short_main"
    side: str = "short"
    ...
    # Entry condition inherited from v3, then modified by v4.
...
def config_dict() -> Dict[str, Any]:
    return asdict(ShortMainV4BaselineConfig())


def add_indicators(df: pd.DataFrame, cfg: ShortMainV4BaselineConfig = ShortMainV4BaselineConfig()) -> pd.DataFrame:
    """Add EMA20, RSI14, ATR14, body, upper_wick, and v4 score columns."""
    out = df.copy()
...
def short_main_v4_entry_mask(df: pd.DataFrame, cfg: ShortMainV4BaselineConfig = ShortMainV4BaselineConfig()) -> pd.Series:
    """
    Return boolean mask for v4 signal candles. Entry itself is next bar open.

    v4 intentionally does not require rsi14 > 77 as a direct gate.
...
def build_short_trade_from_signal(
    df: pd.DataFrame,
    signal_index: int,
    cfg: ShortMainV4BaselineConfig = ShortMainV4BaselineConfig(),
...
def apply_dd_brake_state(current_drawdown_pct: float, freeze_left: int, cfg: ShortMainV4BaselineConfig = ShortMainV4BaselineConfig()) -> int:
    """
...
BASELINE_CONFIG = ShortMainV4BaselineConfig()
