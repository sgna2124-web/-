...
"""
Short main v5 baseline strategy.
Same strategy as short_max v6 because it satisfies MDD < 5% and improves official_cd_value.
"""
...
CFG = ShortMainV5Config()


def config_dict(cfg: ShortMainV5Config = CFG) -> Dict[str, Any]:
    return asdict(cfg)
...
def add_indicators(df: pd.DataFrame, cfg: ShortMainV5Config = CFG) -> pd.DataFrame:
    out = df.copy()
...
def entry_mask(df: pd.DataFrame, cfg: ShortMainV5Config = CFG) -> pd.Series:
...
def build_trade(df: pd.DataFrame, signal_index: int, symbol: str = "", cfg: ShortMainV5Config = CFG) -> Optional[Dict[str, Any]]:
...
def net_short_return(entry: float, exit_price: float, cfg: ShortMainV5Config = CFG) -> float:
...
BASELINE_NOTES = {
    ...
    "critical_rules": ["RSI direct gate disabled", "RSI remains inside short_score", "score_min_short in entry mask", "dd_brake edge_current", "entry next bar open", "fee_per_side=0.0004", "position_fraction=0.01"],
}
