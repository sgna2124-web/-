# -*- coding: utf-8 -*-
"""long_main v12 기준선 코드 레퍼런스.

이 파일은 기준선 식별과 재현 판정 상수를 고정하는 파일이다.
전체 배치 백테스트 엔진은 LONG_MAIN_DEV_V18에서 기준선 exact가 통과한 엔진 구조를 사용한다.
다음 개발 파일에서는 첫 후보를 반드시 LM##_000_LONG_MAIN_V12_EXACT_FROZEN으로 두고 이 상수와 같은 결과를 재현해야 한다.
"""
...
PARENT_VERSION = "long_main/v11"
SIDE = "long"
RESULT_SCOPE = "2025년까지의 데이터 기준"
TRAIN_END_EXCLUSIVE_UTC = "2026-01-01 00:00:00"

# Entry source는 v11 frozen entry다. TP03 gate를 최종 RR 5.20으로 다시 계산하지 않는다.
...
ENTRY_FORMULA = "final_entry = long_main_v11_frozen_entry AND body_atr >= 0.25"
