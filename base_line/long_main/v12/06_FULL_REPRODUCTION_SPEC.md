...
WARMUP_BARS는 120이다. raw mask 루프는 `for i in range(max(21, WARMUP_BARS), n - 1)`로 돈다.
...
## 5. v11 frozen entry source 계산

v12의 entry source는 long_main v11 frozen entry다. TP03 gate는 최종 rr 5.20이 아니라 entry source rr 3.80으로 계산한다.

```python
ENTRY_SOURCE_ATR_STOP = 1.10
ENTRY_SOURCE_RR_TARGET = 3.80

shock_down = raw_shock_down
l01 = raw_l01
shock_balance = raw_shock_reversal_balance
extreme = raw_extreme_reclaim

family_v09 = shock_down | l01 | shock_balance
anchor_extreme = extreme | (rsi14 <= 34.0)
...
5. final exit가 `atr_stop=1.15`, `rr_target=5.20`인지 확인한다.
6. `body_atr >= 0.25`가 entry_source 뒤에 AND로 붙었는지 확인한다.
