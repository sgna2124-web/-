...
다음 long_main 개선은 반드시 v8에서 시작한다.
...
4. official_cd_value > 363.5507495661
5. 단독 재백테스트에서 결과 재현 가능

보조 선호 조건:

- max_conc < 439
- MDD가 현재 1.3412321126보다 낮거나 크게 상승하지 않을 것
- 수수료 민감도 감소
- 승률 하락 보완
...
3. 기준 entry 유지 + `atr_stop`, `rr_target`, `max_hold_bars`, `cooldown_bars` 조정
4. TP03 게이트 강화 또는 완화
5. MDD 5% 미만을 유지하는 범위에서 손익비 개선

## 금지 및 주의 사항

1. 이전 v7을 다음 개선 기준으로 삼지 않는다.
2. v8의 rr_target은 3.20이다. 2.90을 쓰면 v7 계열 재현이 된다.
...
`long_main 현재 기준선은 v8 8V4_V09_V054_extreme_vol18__CH_tp03_risk_rr_plus20__DEV13_risk_stop_110__DEV14_risk_rr_320이다. MDD 5% 미만 조건을 유지하면서 cd_value 363.5507495661 초과를 목표로 개선한다. 기준선 entry_key는 child::orig_V09_extreme_vol18::tp03이며, 청산 파라미터는 atr_stop 1.10, rr_target 3.20, max_hold 21, cooldown 31이다.`
