...
- 공식명: `LONG_MAIN_V3_LM7_070_ATTACK_REMOVE_CP_WITH_RET20_08`
- 개발 후보명: `LM7_070_ATTACK_REMOVE_CP_WITH_RET20_08`
- 기반 전략: `LONG_MAIN_V2_LM4_014_ATTACK_BODY_NOT_HUGE`
...
v3는 v1 기준선의 핵심 진입 조건을 먼저 통과해야 한다.
...
v3에서는 double_flush lookback 자체를 바꾸지 않는다.
...
v2에서 추가로 강화한 `vol_ratio >= 1.45`를 v3에서도 유지한다.

의미:

- reclaim이 거래량을 동반했는지 확인한다.
- 거래량 없는 약한 회복을 제거한다.
- v3에서는 ret20 조건과 결합해 “충분히 빠진 뒤 거래량 동반 회복”만 남긴다.

주의:

- vol_ratio를 과하게 높이면 좋은 거래가 제거될 수 있다.
- 현재 기준선 v3는 1.45를 공식값으로 사용한다.

### 5.2 body_atr <= 1.60

v2에서 도입된 loose chase guard다.

의미:

- 너무 과도하게 튄 반전봉 종가 추격을 줄인다.
- body_atr가 지나치게 큰 봉을 느슨하게 제거해 MDD를 완화한다.

주의:

- body_atr <= 1.45는 실패했다.
- body_atr <= 1.35도 너무 강했다.
- v3에서는 1.60을 유지한다.
...
- 최종 1위인 LM7_070은 이 조건을 포함한다.

주의:

- ret20 <= -0.10은 v3 후보보다 성과가 낮았다.
- ret20 <= -0.12는 너무 강했다.
- 현재 기준선 v3에서는 -0.08을 사용한다.
...
- v7에서 `close_pos >= 0.77`을 제거하고 `ret20 <= -0.08`을 붙인 LM7_070이 전체 1위를 기록했다.
...
v3 기준선을 개발할 때는 다음을 지켜야 한다.

1. `raw_l01_cap_reclaim + double_flush_ok`를 제거하지 않는다.
2. `vol_ratio >= 1.45`, `body_atr <= 1.60`, `ret20 <= -0.08`을 기본값으로 둔다.
3. `close_pos >= 0.77`은 v3 기준선에는 포함하지 않는다.
4. v1 raw의 `close_pos > 0.70`은 반드시 유지한다.
5. 다음 개선안은 v3 기준선 위에 추가하거나 소폭 변형한다.
6. ret20을 -0.12처럼 강하게 조이는 방향은 우선 제외한다.
7. body_atr upper guard를 1.45 이하로 강화하지 않는다.
