...
- long_main 갱신 판정: `long_main_update_candidate = true`

v10은 v9 대비 진입 조건은 유지하고 `rr_target`만 3.50에서 3.80으로 상향한 전략이다. MDD는 1.2943172013524573%로 long_main 기준인 MDD 5% 미만을 충분히 통과한다. V19 탐색 1위가 V20 단독 리테스트에서 trades, wins, losses, cd_value까지 완전 일치했다.
...
long_main 다음 개선 목표는 2025년까지의 데이터 기준 `max_drawdown_pct < 5` 유지와 `official_cd_value > 426.96146593036525` 달성이다.
