...
def print_debug_summary(symbol, df, skip_length):
    ...
    signal_rows = check_df.loc[check_df["short_setup"] == True, [
        "date", "close", "high", "rsi_high", "upper_wick", "body", "close_pos",
...
def run_(prange, hl_period, skip_length, for_check, for_check_nu):
    ...
    for ss in coin_df.symbol:
        ...
        pending_short_signal_close = np.nan
        pending_short_bar = -1
        ...
        df["short_cross"] = (df["rsi_high"].shift(1) > 70) & (df["rsi_high"] <= 70)
        df["wick_cond"] = (df["upper_wick"] > df["body"] * 1.5) & (df["upper_wick"] > df["lower_wick"])
        df["close_cond"] = df["close_pos"] < 0.35
        df["short_setup"] = (
            df["short_cross"] &
            df["wick_cond"] &
            df["close_cond"] &
        ...
        for i in range(skip_length + 1, len(df)):
            ...
            pre_ha_bear = bool(df["ha_strong_bear"].iloc[i])

            if pending_short == 1 and i - pending_short_bar > 10:
                pending_short = 0
                pending_short_sl = np.nan
                pending_short_signal_close = np.nan
                pending_short_bar = -1

            if short_count == 0 and pending_short == 1 and pre_ha_bear:
                entry_price = pre_close
                original_risk = pending_short_sl - pending_short_signal_close

                if original_risk > 0:
                    move_ratio = max(pending_short_signal_close - entry_price, 0) / original_risk
                    rr = 1.0 if move_ratio >= 0.6 else 1.5 if move_ratio >= 0.3 else 2.0
                    final_sl = pending_short_sl - (pending_short_sl - pre_high) * 0.5 if move_ratio >= 0.5 else pending_short_sl
                    ...
                    if final_sl > entry_price:
                        ...
                        short_more = final_sl
                        short_end = entry_price - (final_sl - entry_price) * rr
            ...
            if short_count >= 1:
                ...
                if stop_hit:
                    profit = (short_avr_price - short_more) / short_avr_price * 100
            ...
            if short_count == 0 and pending_short == 0 and bool(df["short_setup"].iloc[i]):
                pending_short = 1
                pending_short_sl = df["high"].iloc[i]
                pending_short_signal_close = df["close"].iloc[i]
                pending_short_bar = i
    ...
    if len(df_sm) > 0:
        ...
        for i in range(len(df_sm)):
            if i == 0 or df_sm.Date.iloc[i] != df_sm.Date.iloc[i - 1]:
                ...
                len_dev = len(dev)
                devide = len_dev if len_dev > 100 else 100
                in_money = money / 100
