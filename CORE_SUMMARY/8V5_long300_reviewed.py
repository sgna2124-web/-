...
TIMEFRAME_LABEL = "5m"
BATCH_LABEL = "8V5_LONG300_REVIEWED"
...
FAMILY_SEEDS = {
    "V51P": {"label": "v51_param_tune", "desc": "v51 parameter tuning"},
    "V51E": {"label": "v51_edge_boost", "desc": "v51 edge boost"},
    "V51C": {"label": "v51_mdd_cover", "desc": "v51 mdd cover"},
}
ANCHOR_TAGS = [
    ("core", "core"),
    ("strict", "strict"),
...
def family_risk_profile(seed: str, a_idx: int, g_idx: int) -> Tuple[float, float, int, int]:
    seed_base = {
        "V51P": (0.88, 2.45, 18, 20),
        "V51E": (0.95, 2.85, 22, 18),
        "V51C": (0.82, 2.10, 16, 28),
    }[seed]
    ...
    for seed, meta in FAMILY_SEEDS.items():
        ...
        for a_idx, (a_code, a_desc) in enumerate(ANCHOR_TAGS, start=1):
            for g_idx, (g_code, g_desc) in enumerate(GUARD_TAGS, start=1):
                ...
                name = f"8V5_{seed}_V{variant_no:03d}_{a_code}_{g_code}"
...
def family_seed_core(seed: str, f: FeaturePack, raw: RawSignals, i: int, score: int) -> bool:
    ...
    if seed == "V51E":
        return age_early and raw.l01_cap_reclaim[i] and drive and score5 and rare18 and (f.body_atr[i] >= 0.28)
    if seed == "V51C":
        return age_wide and raw.l01_cap_reclaim[i] and low_anchor_wide and score4 and rare28 and c1
    return False
...
def family_specific_gate(seed: str, a_idx: int, g_idx: int, f: FeaturePack, raw: RawSignals, i: int, score: int) -> bool:
    ...
    qb = f.close_pos[i] >= 0.78
    qh = f.close_pos[i] >= 0.82
    quiet = f.quiet_ratio[i] <= 0.95
    not_manic18 = vol_not_manic(f, i, 1.80)
    not_manic16 = vol_not_manic(f, i, 1.60)
    ...
    if seed == "V51P":
        ...
        extras = [
            age_early and not_manic18,
            drive and rare18,
            strict and qh,
            low_anchor_wide and drive,
            shock_low and prev,
            prev and qb,
            raw.extreme_reclaim[i] and score5,
        ...
        extras = [
            age_mid and c1,
            drive and rare28,
            strict and not_manic16,
            low_anchor_wide and quiet,
            shock_low and low_anchor,
            prev and strict,
            raw.extreme_reclaim[i] and c0,
            ema and strict and moderate,
            qh and c0,
            score4 and location_soft_point(f, i),
        ]
    else:
        return False
