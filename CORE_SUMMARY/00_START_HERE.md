...
TP03 조건 최신 보정
과거에는 신규 개선안에 TP 기대값 0.3% 이상 진입 조건을 필수로 넣도록 기록했으나, 8V30에서 TP03-only 후보의 결과가 논리적으로 모순되었다.
기준선에 조건을 추가했다면 거래 수는 같거나 줄어야 하는데 long TP03-only 후보에서 거래 수가 폭증했다.
따라서 TP03 결과를 전략 효과로 해석하지 않는다.
CORE_SUMMARY/51_8V30_RESULT_INTERPRETATION_CORRECTION_AND_AUDIT_RULE.md 가 이 보정의 최신 기준이다.
기준선 clone 재현과 수수료/포지션 비중 감사가 끝나기 전까지 TP03은 공통 필수 조건으로 적용하지 않는다.
...
4. 실패 결과도 장단점으로 기록한다.
5. 단, invalid_env는 전략 실패가 아니라 환경/엔진 실패로 분리한다.

다음 백테스트 설계 전 반드시 할 일
1. 최신 CORE_SUMMARY 파일을 번호순으로 읽는다.
2. CORE_SUMMARY/12_STRATEGY_STRENGTHS_WEAKNESSES.md 와 최신 addendum을 읽는다.
3. 기준선이 바뀌었는지 확인한다.
4. 이전과 같은 실패 패턴을 반복하지 않도록 후보 생성 규칙에 반영한다.
5. 백테스트 예상 시간은 1시간 내외가 되도록 조절하되, 캔들 제한으로 비교 기준을 훼손하지 않는다.
6. 캔들 제한은 금지한다. 시간 단축은 후보 수 조정, 축별 우선순위 조정, clone/audit 배치 분리로 해결한다.
7. 기준선 clone 재현과 수수료/포지션 비중 감사가 끝나기 전에는 신규 개선 배치로 넘어가지 않는다.

현재 로컬 저장소 접근 경로
사용자의 컴퓨터에서 깃허브 저장소에 접근하는 경로는 다음이다.
C:\Users\user\Documents\GitHub\-
주의: 이것은 저장소 접근 경로이며, 파이썬 파일 실행 위치와 같다고 가정하면 안 된다.
주의: GitHub 아래에 '-' 폴더가 있는 구조다.
주의: C:\Users\user\Documents\GitHub- 는 틀린 경로다.

경로 처리 행동 규칙
1. 사용자의 로컬 저장소 경로를 출력하거나 코드에 박을 때는 반드시 C:\Users\user\Documents\GitHub\- 로 쓴다.
2. C:\Users\user\Documents\GitHub- 라고 쓰면 오답이다.
3. 파이썬 실행 위치는 별도로 다룬다. 저장소 접근 경로와 실행 경로를 혼동하지 않는다.
4. 데이터 경로는 실행 cwd 또는 명시적 data_root를 기준으로 확인하며, 저장소 접근 경로 아래에 데이터가 있다고 단정하지 않는다.
5. 반복 실수가 발생하면 즉시 CORE_SUMMARY에 행동 규칙을 추가한다.

현재 기준 백테스트 코드
CORE_SUMMARY/run_8v20_embedded_parent_gate_4axis_50_each.py 가 현재 기준 파일이다.
이 파일은 기준선 4개를 내장하고 있다.
단, 8V30 이후에는 해당 기준 파일에서 파생된 improve 실행 경로가 embedded reference 기준선과 동일하게 재현되는지 먼저 감사해야 한다.
기준선 전략의 조건은 개선 과정에서 직접 훼손하지 않는다.
기준선과 같은 환경에서 비교해야 하며, 캔들 제한이나 수수료/포지션 비중 변경으로 비교 기준을 흔들면 안 된다.

마지막 핵심 업데이트
8V30 결과는 전략 실패라기보다 감사 실패 신호다.
다음 단계는 8V31_AUDIT_CLONE_TP03_FEE 형태의 clone sanity + TP03 실제 적용 + 수수료/포지션 비중 감사다.
clone이 기준선과 trades/max_return/MDD를 재현하지 못하면 그 축의 개선을 중단한다.
