...
직후 진행한 6V3_LONG10_REVIEWED에서는 post-signal 패턴 강화형 변형 10개가 전부 승격에 실패했다.
즉 다음 long 개선 방향은 코어 위에 후행 패턴을 덧칠하는 방식이 아니라, 코어는 유지한 채 종목 상태, 위치, 레짐 같은 얇은 전처리 필터를 추가하는 쪽이어야 한다.
...
최근 라운드의 세부 보정 사항은 CORE_SUMMARY/13_6V2_LONG10_STRENGTHS_WEAKNESSES_ADDENDUM.md 와 CORE_SUMMARY/14_6V3_LONG10_STRENGTHS_WEAKNESSES_ADDENDUM.md 를 함께 본다.
...
이번 구간의 핵심 학습은 shock-reclaim 코어는 유지하되, mid reclaim / ema snap / inside pop / hold-break 같은 후행 패턴 강화는 샘플 수를 죽이거나 늦은 추격을 만들어 엣지를 훼손할 수 있다는 점이다.
...
7. CORE_SUMMARY/09_NEXT_CHAT_MANUAL.md
