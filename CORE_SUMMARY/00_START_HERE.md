...
현재 방향성 요약
파이썬 로컬 백테스트 전환 이후 long 쪽에서 처음으로 공식 승격이 발생했다.
...
반면 숏 측은 기존 높은 기준선이 여전히 유지되므로, 다음 단계의 우선순위는 새 long 기준선을 더 개선하는 것이다.
...
이번 구간의 핵심 학습은 shock-reclaim 코어는 유지하되, delayed second push나 quiet/contraction류의 과도한 추가 필터는 성능 악화 또는 0트레이드로 이어질 수 있다는 점이다.
