...
6V3~6V5에서 기존 family를 다듬는 시도는 과선택화, 과다거래, 또는 edge 희석으로 이어졌다.
7V1에서 신규 reversal family의 raw upside가 처음 확인되었고, 7V2와 7V3에서 이 신규 family가 제한형 구조에서도 실제로 cd 110대까지 도달할 수 있다는 점이 확인되었다.
7V3에서는 40전략 배치의 실행 시간도 크게 단축되었다. 공통 feature/raw를 심볼당 한 번만 계산하는 구조는 유지해야 한다.
7V4에서는 candidate_cd_win_mdd_fail 규칙을 도입했고, L09 family 안에서 기준선보다 높은 cd를 가진 전략이 실제로 등장했다.
...
다만 두 전략 모두 MDD 5%를 넘겨 공식 승격은 실패했다.
한편 MDD<5 구간의 best는 여전히 7V4_L09_V02_beartrap_extreme_synergy | cd_value 112.8990 수준이다.
즉 현재 long 설계의 최우선 과제는 L09 beartrap family의 높은 raw/cd edge를 유지한 채 MDD를 5% 아래로 끌어내리는 것이다.
L07 wick family는 2순위 보조 축으로 유지한다.
L10 balance와 L04 engulf는 보조 참조 family로 둔다.
...
최근 라운드의 세부 보정 사항은 CORE_SUMMARY/13_6V2_LONG10_STRENGTHS_WEAKNESSES_ADDENDUM.md 부터 CORE_SUMMARY/20_7V4_LONG40_STRENGTHS_WEAKNESSES_ADDENDUM.md 까지를 함께 본다.
...
7V1_LONG10_REVIEWED에서는 broad same-bar reversal 계열이 raw edge는 있었지만 MDD가 너무 높았다.
7V2_LONG40_REVIEWED에서는 L09 beartrap family와 L07 wick family가 MDD<5 구간에서도 cd 110대까지 올라올 수 있음을 보여줬다.
7V3_LONG40_REVIEWED에서는 이 방향이 유지되었고, 동시에 40전략 배치의 실행 시간도 크게 단축되었다.
7V4_LONG40_REVIEWED에서는 공식 기준선 교체는 없었지만, L09 family 안에서 candidate_cd_win_mdd_fail 두 개가 처음으로 명확하게 등장했다.
이번 구간의 핵심 학습은 다음과 같다.
1) L09 beartrap family가 가장 유망하다.
2) L07 wick family가 안정적 2순위다.
3) next step은 L09의 높은 cd를 유지하면서 MDD를 낮추는 것이다.
...
13. CORE_SUMMARY/09_NEXT_CHAT_MANUAL.md
