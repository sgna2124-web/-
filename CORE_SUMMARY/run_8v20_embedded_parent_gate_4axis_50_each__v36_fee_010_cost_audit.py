...
BATCH = "8V20_EMBEDDED_PARENT_GATE_4AXIS_50_EACH"
POS_FRAC = 0.01
COST_PCT = 0.10
WARMUP = 140
MIN_BARS = 260
...
def variants_for_axis(axis: str) -> List[Spec]:
    ...
    for j in range(50):
        n = j + 1
        if axis == "long_main":
            # 6V2 장점: 적은 거래수와 낮은 MDD. 단점: 수익률 한계.
            if j < 10:
                out.append(replace_spec(base, name=f"8V20_long_main_{n:03d}_rr_lift", kind="improve", tag="rr_lift",
                    rr=base.rr + 0.04 * (j + 1), hold=base.hold + (j % 3) * 2, trail_r=base.trail_r + 0.08 * (j % 4)))
            elif j < 20:
                out.append(replace_spec(base, name=f"8V20_long_main_{n:03d}_quality_keep", kind="improve", tag="quality_keep",
                    close_pos_min=base.close_pos_min + 0.01 * ((j - 10) % 5), wick_min=base.wick_min + 0.04 * ((j - 10) % 5),
                    vol_min=base.vol_min + 0.03 * ((j - 10) % 4)))
            elif j < 30:
                out.append(replace_spec(base, name=f"8V20_long_main_{n:03d}_loss_brake", kind="improve", tag="loss_brake",
                    post_loss_after=2 + ((j - 20) % 3), post_loss_bars=12 + ((j - 20) % 5) * 4, cooldown=base.cooldown + ((j - 20) % 4) * 2))
            elif j < 40:
                out.append(replace_spec(base, name=f"8V20_long_main_{n:03d}_shock_guard", kind="improve", tag="shock_guard",
                    shock_ret5=base.shock_ret5 + 0.004 * ((j - 30) % 5), shock_range=base.shock_range - 0.006 * ((j - 30) % 4),
                    sym_dd_pct=base.sym_dd_pct - 0.006 * ((j - 30) % 4)))
            else:
                out.append(replace_spec(base, name=f"8V20_long_main_{n:03d}_mixed_edge", kind="improve", tag="mixed_edge",
                    rr=base.rr + 0.05 * ((j - 40) % 5), close_pos_min=base.close_pos_min + 0.015 * ((j - 40) % 4),
                    cooldown=base.cooldown + ((j - 40) % 4) * 3, trail_atr=base.trail_atr - 0.05 * ((j - 40) % 4)))
        elif axis == "long_max":
            # V51 장점: raw edge. 단점: MDD 6.7대. 최소 억제로 MDD를 5 아래로 낮추는 축.
            if j < 10:
                out.append(replace_spec(base, name=f"8V20_long_max_{n:03d}_mdd_clip", kind="improve", tag="mdd_clip",
                    sym_dd_pct=base.sym_dd_pct - 0.008 * ((j % 5) + 1), sym_dd_bars=base.sym_dd_bars + (j % 4) * 4))
            elif j < 20:
                out.append(replace_spec(base, name=f"8V20_long_max_{n:03d}_strict_rare22", kind="improve", tag="strict_rare22",
                    require_strict=(j % 2 == 0), require_lowanchor=(j % 3 == 0), close_pos_min=base.close_pos_min + 0.01 * ((j - 10) % 5)))
            elif j < 30:
                out.append(replace_spec(base, name=f"8V20_long_max_{n:03d}_exit_tune", kind="improve", tag="exit_tune",
                    rr=base.rr - 0.05 * ((j - 20) % 5), hold=base.hold - ((j - 20) % 4) * 2,
                    be_r=base.be_r - 0.05 * ((j - 20) % 4), trail_r=base.trail_r - 0.06 * ((j - 20) % 4)))
            elif j < 40:
                out.append(replace_spec(base, name=f"8V20_long_max_{n:03d}_density_cool", kind="improve", tag="density_cool",
                    cooldown=base.cooldown + ((j - 30) % 6) * 3, post_loss_after=3, post_loss_bars=10 + ((j - 30) % 5) * 4))
            else:
                out.append(replace_spec(base, name=f"8V20_long_max_{n:03d}_official_mix", kind="improve", tag="official_mix",
                    sym_dd_pct=base.sym_dd_pct - 0.010 * ((j - 40) % 4), rr=base.rr - 0.04 * ((j - 40) % 5),
                    require_lowanchor=(j % 2 == 0), close_pos_min=base.close_pos_min + 0.012 * ((j - 40) % 4)))
        elif axis == "short_main":
            # short_beh_dd_brake 장점: 공식 MDD<5의 높은 성장. 단점: 밀집/연속손실 구간 리스크.
            if j < 12:
                out.append(replace_spec(base, name=f"8V20_short_main_{n:03d}_dd_brake_tune", kind="improve", tag="dd_brake_tune",
                    sym_dd_pct=base.sym_dd_pct - 0.010 * ((j % 6) + 1), sym_dd_bars=base.sym_dd_bars + (j % 4) * 3))
            elif j < 24:
                out.append(replace_spec(base, name=f"8V20_short_main_{n:03d}_score_edge", kind="improve", tag="score_edge",
                    dev_short=base.dev_short + 0.001 * ((j - 12) % 6), rsi_short_min=base.rsi_short_min + ((j - 12) % 5),
                    wick_short_min=base.wick_short_min + 0.03 * ((j - 12) % 4)))
            elif j < 36:
                out.append(replace_spec(base, name=f"8V20_short_main_{n:03d}_loss_pause", kind="improve", tag="loss_pause",
                    post_loss_after=3 + ((j - 24) % 3), post_loss_bars=12 + ((j - 24) % 6) * 4, cooldown=(j - 24) % 4))
            elif j < 44:
                out.append(replace_spec(base, name=f"8V20_short_main_{n:03d}_exit_compact", kind="improve", tag="exit_compact",
                    rr=base.rr - 0.12 * ((j - 36) % 5), hold=base.hold - ((j - 36) % 5) * 8,
                    trail_r=base.trail_r - 0.08 * ((j - 36) % 4)))
            else:
                out.append(replace_spec(base, name=f"8V20_short_main_{n:03d}_hybrid_guard", kind="improve", tag="hybrid_guard",
                    dev_short=base.dev_short + 0.002 * ((j - 44) % 4), rsi_short_min=base.rsi_short_min + ((j - 44) % 4),
                    sym_dd_pct=base.sym_dd_pct - 0.012 * ((j - 44) % 4), rr=base.rr - 0.10 * ((j - 44) % 3)))
        elif axis == "short_max":
            # short_only_reference_1x 장점: raw upside. 단점: MDD 7.7대. edge 훼손 없이 DD 압축.
            if j < 12:
                out.append(replace_spec(base, name=f"8V20_short_max_{n:03d}_score_lift", kind="improve", tag="score_lift",
                    dev_short=base.dev_short + 0.001 * ((j % 6) + 1), rsi_short_min=base.rsi_short_min + (j % 6),
                    wick_short_min=base.wick_short_min + 0.025 * (j % 5)))
            elif j < 24:
                out.append(replace_spec(base, name=f"8V20_short_max_{n:03d}_mdd_brake", kind="improve", tag="mdd_brake",
                    sym_dd_pct=base.sym_dd_pct - 0.015 * ((j - 12) % 6), sym_dd_bars=base.sym_dd_bars + ((j - 12) % 4) * 4))
            elif j < 34:
                out.append(replace_spec(base, name=f"8V20_short_max_{n:03d}_risk_compact", kind="improve", tag="risk_compact",
                    rr=base.rr - 0.18 * ((j - 24) % 6), hold=base.hold - ((j - 24) % 6) * 12,
                    be_r=base.be_r - 0.04 * ((j - 24) % 4)))
            elif j < 44:
                out.append(replace_spec(base, name=f"8V20_short_max_{n:03d}_loss_guard", kind="improve", tag="loss_guard",
                    post_loss_after=3 + ((j - 34) % 3), post_loss_bars=10 + ((j - 34) % 6) * 4, cooldown=(j - 34) % 5))
            else:
                out.append(replace_spec(base, name=f"8V20_short_max_{n:03d}_official_mix", kind="improve", tag="official_mix",
                    dev_short=base.dev_short + 0.002 * ((j - 44) % 4), rsi_short_min=base.rsi_short_min + ((j - 44) % 5),
                    sym_dd_pct=base.sym_dd_pct - 0.014 * ((j - 44) % 4), rr=base.rr - 0.14 * ((j - 44) % 4)))
        else:
            raise ValueError(axis)
    return out
