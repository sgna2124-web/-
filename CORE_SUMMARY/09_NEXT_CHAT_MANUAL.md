...
3. 8V11 lx_deep_absorb/lx_trend_repair는 단독 부모보다 MDD 압축 보조 모듈로 혼합한다.
...
6. short는 다음 라운드에서 원본 short edge를 보존하는 방향으로 재설계해야 하며, 8V11 short 방식은 반복하지 않는다.

다음 전략 설계 방향
새로운 family 탐색보다 V51 및 8V10 후보 심화가 우선이다.
8V7 top 전략이나 8V8 top 전략을 직접 부모로 쓰지 않는다.
8V11 long top은 직접 부모보다 보조 모듈로 쓴다.
...
post_loss_brake, soft_safe_mix, raw_official_conversion, lx_deep_absorb는 유효성이 확인됐지만 hard filter가 아니라 부분 모듈로만 이식한다.
