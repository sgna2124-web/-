...
def variants_for_axis(axis):
    ...
    def _tp03(s):
        ...
        if missing:
            raise RuntimeError("TP03 적용에 필요한 Spec 필드가 없습니다: " + ",".join(sorted(missing)))
    ...
    def _make(name, tag, use_tp03=False, **changes):
    ...
    trail0 = float(_get(base, "trail_atr", 2.0))
    ...
    tp_only_name = {
        "long_main": "8V30_long_main_001_tp03_only_probe",
        "long_max": "8V30_long_max_001_tp03_only_probe",
        "short_main": "8V30_short_main_001_tp03_only_probe",
        "short_max": "8V30_short_max_001_tp03_only_probe",
    }[axis]
    out = [_make(tp_only_name, "tp03_only_probe_on_baseline_spec", use_tp03=True)]

    if axis == "long_main":
        out.append(_make(
            "8V30_long_main_002_loosen_reclaim_wick_close_range",
            "long_loosen_same_family_wick_close_range_no_tp03",
            use_tp03=False,
            require_green=True,
            ...
            ema_guard="none",
            close_pos_min=max(0.0, close0 - 0.030),
            wick_min=max(0.0, wick0 - 0.050),
            upper_wick_max=min(1.0, upper0 + 0.080),
            vol_min=max(0.0, vol0 - 0.080),
            body_min=max(0.0, body0 - 0.030),
            range20_max=min(1.0, range20_0 + 0.030),
            atrp_min=max(0.0, atrp_min0 - 0.0002),
            rr=max(1.20, rr0 - 0.020),
            hold=hold0 + 2,
        ))
        return out
    ...
    if axis == "long_max":
        out.append(_make(
            "8V30_long_max_002_loosen_rare22_range_vol_upper",
            "long_rare22_loosen_range_vol_upper_no_tp03",
            use_tp03=False,
            require_green=True,
            require_reclaim=True,
            require_rare22=True,
            ...
            require_strict=False,
            ema_guard="none",
            close_pos_min=max(0.0, close0 - 0.020),
            upper_wick_max=min(1.0, upper0 + 0.100),
            wick_min=max(0.0, wick0 - 0.040),
            vol_min=max(0.0, vol0 - 0.100),
            body_min=max(0.0, body0 - 0.030),
            range20_max=min(1.0, range20_0 + 0.050),
            atrp_min=max(0.0, atrp_min0 - 0.0002),
            rr=max(1.20, rr0 - 0.030),
            hold=hold0 + 2,
        ))
        return out
    ...
    if axis == "short_main":
        out.append(_make(
            "8V30_short_main_002_no_tp03_profit_keep_loss_trim",
            "short_keep_dev_rsi_add_soft_loss_trim_no_tp03",
            use_tp03=False,
            dev_short=dev0 + 0.0004,
            rsi_short_min=rsi0 + 0.2,
            wick_short_min=wick_short0 + 0.040,
            close_pos_short_max=max(0.0, close_short0 - 0.020),
            shock_ret5=shock5_0 + 0.0015,
            shock_range=shock_range0 + 0.0015,
            shock_close_pos=max(0.0, shock_close0 - 0.015),
            rr=max(1.20, rr0 - 0.060),
            hold=max(30, hold0 - 4),
            trail_atr=max(1.20, trail0 - 0.060),
            cooldown=cooldown0 + 1,
            post_loss_after=5,
            post_loss_bars=8,
            sym_dd_pct=0.090,
            sym_dd_bars=14,
    ...
    if axis == "short_max":
        out.append(_make(
            "8V30_short_max_002_no_tp03_blowoff_loss_trim",
            "short_blowoff_keep_edge_add_soft_loss_trim_no_tp03",
            use_tp03=False,
            dev_short=dev0 + 0.0003,
            rsi_short_min=rsi0 + 0.2,
            wick_short_min=wick_short0 + 0.030,
            close_pos_short_max=max(0.0, close_short0 - 0.015),
            shock_ret5=shock5_0 + 0.0012,
            shock_range=shock_range0 + 0.0012,
            shock_close_pos=max(0.0, shock_close0 - 0.012),
            rr=max(1.20, rr0 - 0.040),
            hold=max(30, hold0 - 3),
            trail_atr=max(1.20, trail0 - 0.050),
            cooldown=cooldown0 + 1,
            post_loss_after=6,
            post_loss_bars=8,
            sym_dd_pct=0.100,
