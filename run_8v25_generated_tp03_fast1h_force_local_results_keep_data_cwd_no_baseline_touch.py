...
def variants_for_axis(axis):
    # 8V25: 1시간 예산형 fast lane.
    # 기본은 각 축 1개씩 총 4개 개선안만 생성한다.
    # LOB_FAST_AXIS가 특정 축이면 해당 축만 생성하고 나머지는 빈 목록으로 둔다.
    ...
    if axis == "long_main":
        return [_make(
            "8V25_long_main_001_balanced_tp03_fast1h",
            "balanced_tp03_fast1h",
            rr=rr0 + 0.08,
            hold=hold0 + 4,
            trail_atr=max(1.20, trail0 - 0.04),
            close_pos_min=close0 + 0.008,
            vol_min=vol0 + 0.018,
    ...
    if axis == "long_max":
        return [_make(
            "8V25_long_max_001_mdd_clip_tp03_fast1h",
            "mdd_clip_tp03_fast1h",
            sym_dd_pct=0.032,
            sym_dd_bars=14,
            rr=max(1.20, rr0 - 0.04),
            hold=max(8, hold0 - 4),
            trail_atr=max(1.20, trail0 - 0.05),
            close_pos_min=close0 + 0.008,
            vol_min=vol0 + 0.012,
    ...
    if axis == "short_main":
        return [_make(
            "8V25_short_main_001_stable_dd_tp03_fast1h",
            "stable_dd_tp03_fast1h",
            sym_dd_pct=0.030,
            sym_dd_bars=10,
            post_loss_after=3,
            post_loss_bars=14,
            rr=max(1.20, rr0 - 0.06),
            hold=max(20, hold0 - 5),
            trail_atr=max(1.20, trail0 - 0.04),
            dev_short=float(_get(base, "dev_short", 0.0)) + 0.00060,
            rsi_short_min=float(_get(base, "rsi_short_min", 0.0)) + 0.50,
        )]

    if axis == "short_max":
        return [_make(
            "8V25_short_max_001_blowoff_guard_tp03_fast1h",
            "blowoff_guard_tp03_fast1h",
            sym_dd_pct=0.034,
            sym_dd_bars=12,
            post_loss_after=3,
            post_loss_bars=14,
            ...
            hold=max(20, hold0 - 6),
            trail_atr=max(1.20, trail0 - 0.05),
            dev_short=float(_get(base, "dev_short", 0.0)) + 0.00055,
            rsi_short_min=float(_get(base, "rsi_short_min", 0.0)) + 0.50,
            ret5_ceil=max(0.080, float(_get(base, "ret5_ceil", 0.45)) - 0.018),
            shock_range=max(0.090, float(_get(base, "shock_range", 0.24)) - 0.012),
        )]
