...
def variants_for_axis(axis):
    ...
    # 시간은 max-bars가 아니라 개선안 수로만 조절한다.
    # 8V25의 거래 기근을 피하기 위해 진입 필터 강화는 넣지 않는다.
    if axis == "long_main":
        return [
            _make("8V27_long_main_001_rr_only_tp03_nocap", "rr_only_tp03_nocap", rr=rr0 + 0.025)
        ]

    if axis == "long_max":
        return [
            _make("8V27_long_max_001_exit_only_tp03_nocap", "exit_only_tp03_nocap", hold=max(8, hold0 - 2))
        ]

    if axis == "short_main":
        return [
            _make("8V27_short_main_001_exit_only_tp03_nocap", "exit_only_tp03_nocap", hold=max(20, hold0 - 3))
        ]

    if axis == "short_max":
        return [
            _make("8V27_short_max_001_trail_only_tp03_nocap", "trail_only_tp03_nocap", trail_atr=max(1.20, trail0 - 0.025))
        ]
