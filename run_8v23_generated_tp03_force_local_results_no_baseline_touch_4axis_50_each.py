...
BATCH = "8V23_TP03_FORCE_LOCAL_RESULTS_NO_BASELINE_TOUCH_4AXIS_50_EACH"
...
def variants_for_axis(axis):
    ...
    for j in range(50):
        ...
        if axis == "long_main":
            if j < 10:
                out.append(_make(f"8V23_long_main_{n:03d}_rr_hold_lift", "rr_hold_lift_tp03", rr=rr0 + 0.025*(k+1), hold=hold0 + 2*(k%4), trail_atr=max(1.20, trail0 - 0.025*(k%5))))
            elif j < 20:
                out.append(_make(f"8V23_long_main_{n:03d}_quality_reclaim", "quality_reclaim_tp03", close_pos_min=close0 + 0.005*(k%6), wick_min=float(_get(base, "wick_min", 0.0)) + 0.020*(k%5), vol_min=vol0 + 0.015*(k%5), reclaim_buffer=float(_get(base, "reclaim_buffer", 0.0)) + 0.00010*(k%5)))
            elif j < 30:
                out.append(_make(f"8V23_long_main_{n:03d}_soft_brake", "soft_brake_tp03", post_loss_after=2+(k%3), post_loss_bars=8+3*(k%5), cooldown=cooldown0+(k%4), fail_bars=max(4, int(_get(base, "fail_bars", 8))-(k%3)), fail_pnl=float(_get(base, "fail_pnl", -0.4))+0.02*(k%5)))
            elif j < 40:
                out.append(_make(f"8V23_long_main_{n:03d}_shock_range_guard", "shock_range_guard_tp03", shock_ret5=float(_get(base, "shock_ret5", 0.0))+0.0025*(k%6), shock_range=max(0.050, float(_get(base, "shock_range", 0.20))-0.0035*(k%5)), shock_close_pos=float(_get(base, "shock_close_pos", close0))+0.010*(k%5), range20_max=max(0.080, range20_0-0.004*(k%5))))
            else:
                out.append(_make(f"8V23_long_main_{n:03d}_balanced_mix", "balanced_mix_tp03", rr=rr0+0.020*(k%6), hold=hold0+2*(k%3), close_pos_min=close0+0.004*(k%5), vol_min=vol0+0.012*(k%6), cooldown=cooldown0+(k%3)))

        elif axis == "long_max":
            if j < 10:
                out.append(_make(f"8V23_long_max_{n:03d}_mdd_soft_clip", "mdd_soft_clip_tp03", sym_dd_pct=0.025+0.0035*(k%6), sym_dd_bars=8+4*(k%5), cooldown=cooldown0+(k%3)))
            elif j < 20:
                out.append(_make(f"8V23_long_max_{n:03d}_edge_quality", "edge_quality_tp03", close_pos_min=close0+0.004*(k%6), upper_wick_max=max(0.70, float(_get(base, "upper_wick_max", 0.95))-0.012*(k%5)), vol_min=vol0+0.012*(k%5), require_lowanchor=(k in {3, 7})))
            elif j < 30:
                out.append(_make(f"8V23_long_max_{n:03d}_exit_compress", "exit_compress_tp03", rr=max(1.20, rr0-0.020*(k%7)), hold=max(8, hold0-2*(k%5)), trail_atr=max(1.20, trail0-0.035*(k%5)), fail_bars=8+(k%4), fail_pnl=-0.45+0.025*(k%5)))
            elif j < 40:
                out.append(_make(f"8V23_long_max_{n:03d}_loss_cluster_guard", "loss_cluster_guard_tp03", post_loss_after=2+(k%4), post_loss_bars=8+3*(k%6), cooldown=cooldown0+(k%4), range20_max=max(0.100, range20_0-0.005*(k%5))))
            else:
                out.append(_make(f"8V23_long_max_{n:03d}_official_balance", "official_balance_tp03", sym_dd_pct=0.028+0.0035*(k%5), sym_dd_bars=10+4*(k%5), rr=max(1.20, rr0-0.015*(k%6)), close_pos_min=close0+0.004*(k%5), require_lowanchor=(k%4==0)))

        elif axis == "short_main":
            if j < 10:
                out.append(_make(f"8V23_short_main_{n:03d}_dd_brake_light", "dd_brake_light_tp03", sym_dd_pct=0.026+0.0035*(k%6), sym_dd_bars=6+3*(k%5), post_loss_after=3+(k%3), post_loss_bars=8+3*(k%5)))
            elif j < 20:
                out.append(_make(f"8V23_short_main_{n:03d}_overheat_quality", "overheat_quality_tp03", dev_short=float(_get(base, "dev_short", 0.0))+0.00045*(k%7), rsi_short_min=float(_get(base, "rsi_short_min", 0.0))+0.45*(k%6), wick_short_min=float(_get(base, "wick_short_min", 0.0))+0.012*(k%6)))
            elif j < 30:
                out.append(_make(f"8V23_short_main_{n:03d}_exit_keep_edge", "exit_keep_edge_tp03", rr=max(1.20, rr0-0.045*(k%7)), hold=max(20, hold0-5*(k%6)), trail_atr=max(1.20, trail0-0.030*(k%6))))
            elif j < 40:
                out.append(_make(f"8V23_short_main_{n:03d}_spike_guard", "spike_guard_tp03", ret3_ceil=max(0.050, float(_get(base, "ret3_ceil", 0.30))-0.005*(k%6)), ret5_ceil=max(0.080, float(_get(base, "ret5_ceil", 0.40))-0.007*(k%6)), shock_ret5=max(0.050, float(_get(base, "shock_ret5", 0.16))-0.0035*(k%6)), shock_range=max(0.080, float(_get(base, "shock_range", 0.20))-0.0035*(k%5))))
            else:
                out.append(_make(f"8V23_short_main_{n:03d}_hybrid_stable", "hybrid_stable_tp03", dev_short=float(_get(base, "dev_short", 0.0))+0.00055*(k%5), rsi_short_min=float(_get(base, "rsi_short_min", 0.0))+0.35*(k%5), rr=max(1.20, rr0-0.035*(k%6)), sym_dd_pct=0.028+0.0035*(k%4), post_loss_after=3+(k%3), post_loss_bars=8+4*(k%4)))

        elif axis == "short_max":
            if j < 10:
                out.append(_make(f"8V23_short_max_{n:03d}_mdd_gate", "mdd_gate_tp03", sym_dd_pct=0.030+0.004*(k%6), sym_dd_bars=8+3*(k%6), post_loss_after=3+(k%4), post_loss_bars=8+3*(k%6)))
            elif j < 20:
                out.append(_make(f"8V23_short_max_{n:03d}_quality_squeeze", "quality_squeeze_tp03", dev_short=float(_get(base, "dev_short", 0.0))+0.00050*(k%7), rsi_short_min=float(_get(base, "rsi_short_min", 0.0))+0.45*(k%7), wick_short_min=float(_get(base, "wick_short_min", 0.0))+0.014*(k%6), close_pos_short_max=max(0.50, float(_get(base, "close_pos_short_max", 0.95))-0.012*(k%7))))
            elif j < 30:
                out.append(_make(f"8V23_short_max_{n:03d}_exit_mdd_balance", "exit_mdd_balance_tp03", rr=max(1.20, rr0-0.055*(k%8)), hold=max(20, hold0-6*(k%7)), trail_atr=max(1.20, trail0-0.040*(k%7))))
            elif j < 40:
                out.append(_make(f"8V23_short_max_{n:03d}_blowoff_guard", "blowoff_guard_tp03", ret3_ceil=max(0.050, float(_get(base, "ret3_ceil", 0.30))-0.006*(k%7)), ret5_ceil=max(0.080, float(_get(base, "ret5_ceil", 0.45))-0.008*(k%7)), ret10_ceil=max(0.100, float(_get(base, "ret10_ceil", 0.60))-0.010*(k%7)), shock_ret5=max(0.060, float(_get(base, "shock_ret5", 0.18))-0.005*(k%7)), shock_range=max(0.090, float(_get(base, "shock_range", 0.24))-0.005*(k%6))))
            else:
                out.append(_make(f"8V23_short_max_{n:03d}_raw_preserve_guard", "raw_preserve_guard_tp03", dev_short=float(_get(base, "dev_short", 0.0))+0.00045*(k%6), rsi_short_min=float(_get(base, "rsi_short_min", 0.0))+0.35*(k%6), rr=max(1.20, rr0-0.040*(k%7)), sym_dd_pct=0.032+0.0035*(k%5), sym_dd_bars=8+4*(k%5), post_loss_after=3+(k%3), post_loss_bars=8+4*(k%5)))
        else:
            raise ValueError(axis)
