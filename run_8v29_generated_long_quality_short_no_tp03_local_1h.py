...
def variants_for_axis(axis):
    # 8V29: long은 TP03 유지 + 기준선 원형 위에 강한 품질 조건 추가.
    # short는 TP03 제거 + 기존 과열 조건군의 일부 대체/추가/완화.
    from dataclasses import fields as _dc_fields, is_dataclass as _dc_is_dataclass, replace as _dc_replace
    import os as _os
    selected = _os.environ.get("LOB_FAST_AXIS", "all").strip()
    if selected not in ("all", axis):
        return []
    base = base_spec(axis)
    def _field_names(s):
    ...
    def _safe_replace(s, **changes):
        ...
        for k, v in filtered.items():
            setattr(obj, k, v)
        return obj
    def _tp03_long_only(s):
        required = {"stop_atr", "rr", "atrp_min"}
        missing = required - _field_names(s)
        if missing:
            raise RuntimeError("long TP03 적용에 필요한 Spec 필드가 없습니다: " + ",".join(sorted(missing)))
    ...
    def _make(name, tag, use_tp03, **changes):
    ...
    atrp_min0 = float(_get(base, "atrp_min", 0.0))
    atrp_max0 = float(_get(base, "atrp_max", 1.0))
    range20_0 = float(_get(base, "range20_max", 1.0))
    vol0 = float(_get(base, "vol_min", 0.0))
    body0 = float(_get(base, "body_min", 0.0))
    close0 = float(_get(base, "close_pos_min", 0.0))
    wick0 = float(_get(base, "wick_min", 0.0))
    upper0 = float(_get(base, "upper_wick_max", 1.0))
    dev0 = float(_get(base, "dev_short", 0.0))
    rsi0 = float(_get(base, "rsi_short_min", 0.0))
    close_short0 = float(_get(base, "close_pos_short_max", 1.0))
    wick_short0 = float(_get(base, "wick_short_min", 0.0))
    shock5_0 = float(_get(base, "shock_ret5", 0.0))
    shock_range0 = float(_get(base, "shock_range", 0.0))
    shock_close0 = float(_get(base, "shock_close_pos", 1.0))
    trail_atr0 = float(_get(base, "trail_atr", 2.0))
    if axis == "long_main":
        return [_make(
            "8V29_long_main_001_shocklow_reclaim_quality_stack_tp03",
            "long_keep_shocklow_reclaim_add_quality_stack_tp03",
            True,
            require_green=True, require_reclaim=True, require_shocklow=True, require_lowanchor=True, require_strict=True, ema_guard="soft",
            close_pos_min=min(0.92, close0 + 0.060), wick_min=wick0 + 0.100, upper_wick_max=max(0.55, upper0 - 0.140),
            vol_min=vol0 + 0.180, body_min=body0 + 0.060, range20_max=max(0.080, range20_0 - 0.050),
            atrp_min=atrp_min0 + 0.0004, atrp_max=max(0.035, atrp_max0 - 0.012), rr=rr0 + 0.020, cooldown=cooldown0 + 2,
        )]
    if axis == "long_max":
        return [_make(
            "8V29_long_max_001_rare22_quality_stack_tp03",
            "long_keep_rare22_add_strict_lowanchor_quality_stack_tp03",
            True,
            require_green=True, require_reclaim=True, require_rare22=True, require_lowanchor=True, require_strict=True, require_shocklow=True, ema_guard="soft",
            close_pos_min=min(0.94, close0 + 0.090), wick_min=max(wick0, 0.120), upper_wick_max=max(0.50, upper0 - 0.260),
            vol_min=vol0 + 0.280, body_min=body0 + 0.090, range20_max=max(0.090, range20_0 - 0.090),
            atrp_min=atrp_min0 + 0.0007, atrp_max=max(0.040, atrp_max0 - 0.025), rr=max(1.40, rr0 - 0.020),
            hold=max(10, hold0 - 2), cooldown=cooldown0 + 4, sym_dd_pct=0.060, sym_dd_bars=24,
        )]
    if axis == "short_main":
        return [_make(
            "8V29_short_main_001_no_tp03_wick_to_sweep_reject_guard",
            "short_remove_tp03_replace_wick_with_sweep_reject_guard",
            False,
            dev_short=dev0 + 0.0008, rsi_short_min=rsi0 + 0.5, wick_short_min=max(0.90, wick_short0 - 0.180),
            close_pos_short_max=min(close_short0, 0.900), require_upper_sweep=True, require_ema_reject=True,
            shock_ret5=shock5_0 + 0.004, shock_range=shock_range0 + 0.004, shock_close_pos=min(shock_close0, 0.900),
            rr=max(4.80, rr0 - 0.120), hold=max(120, hold0 - 8), post_loss_after=4, post_loss_bars=14,
            sym_dd_pct=0.075, sym_dd_bars=18,
        )]
    if axis == "short_max":
        return [_make(
            "8V29_short_max_001_no_tp03_blowoff_reject_mdd_guard",
            "short_remove_tp03_keep_blowoff_add_reject_mdd_guard",
            False,
            dev_short=dev0 + 0.0005, rsi_short_min=rsi0 + 0.5, wick_short_min=max(1.00, wick_short0 - 0.120),
            close_pos_short_max=min(close_short0, 0.910), require_upper_sweep=True, require_ema_reject=True,
            shock_ret5=shock5_0 + 0.004, shock_range=shock_range0 + 0.004, shock_close_pos=min(shock_close0, 0.910),
            rr=max(5.00, rr0 - 0.080), hold=max(130, hold0 - 6), trail_atr=max(1.60, trail_atr0 - 0.100),
            post_loss_after=4, post_loss_bars=12, sym_dd_pct=0.085, sym_dd_bars=20,
        )]
    raise ValueError(axis)
