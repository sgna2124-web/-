...
def variants_for_axis(axis):
    # 8V28: 기준선 로컬 변형형.
    # 목표는 새 전략 발명이 아니라 기준선 조건군의 작은 추가/대체/완화/강화 조합이다.
    # 캔들 제한 없음. 시간은 후보 수로만 조절한다.
    # 기준선 조건/base_spec/BASELINES/entry-exit/parent gate는 수정하지 않는다.
    ...
    # 8V27 결과:
    # - long_main/long_max는 거래 폭증. 따라서 진입 확장 금지, 기준선 내 추가 제한 중심.
    # - short_main/short_max는 거래 기근. 따라서 과도한 제한 금지, 비슷한 성격의 약한 대체/완화 중심.
    # - 각 축 1개씩 총 4개만 실행해 전체 데이터 기준 약 1시간을 목표로 한다.

    if axis == "long_main":
        return [
            _make(
                "8V28_long_main_001_reclaim_lowanchor_local_tp03",
                "add_lowanchor_small_rr_tp03",
                require_lowanchor=True,
                rr=rr0 + 0.015,
                close_pos_min=close0 + 0.004,
            )
        ]
    ...
    if axis == "long_max":
        return [
            _make(
                "8V28_long_max_001_rare22_lowanchor_exit_local_tp03",
                "add_lowanchor_soft_exit_tp03",
                require_lowanchor=True,
                upper_wick_max=max(0.70, upper0 - 0.020),
                hold=max(8, hold0 - 1),
            )
        ]
    ...
    if axis == "short_main":
        return [
            _make(
                "8V28_short_main_001_dev_rsi_soft_replace_tp03",
                "soft_replace_dev_rsi_keep_quality_tp03",
                dev_short=max(0.0, dev0 - 0.0005),
                rsi_short_min=max(0.0, rsi0 - 0.4),
                close_pos_short_max=min(1.0, close_short0 - 0.010),
                rr=max(1.20, rr0 - 0.04),
            )
        ]
    ...
    if axis == "short_max":
        return [
            _make(
                "8V28_short_max_001_blowoff_soft_replace_tp03",
                "soft_replace_blowoff_keep_wick_tp03",
                dev_short=max(0.0, dev0 - 0.0005),
                rsi_short_min=max(0.0, rsi0 - 0.4),
                wick_short_min=max(0.0, wick_short0 - 0.020),
                shock_ret5=max(0.0, shock5_0 - 0.003),
                shock_range=max(0.0, shock_range0 - 0.004),
            )
        ]
